...
import { cleaningOperationsRecordsTable } from "./schema/cleaningOperations.js";
...
function upsertRecord(
  ...
  const existing = db.select().from(cleaningOperationsRecordsTable)
    .where(and(
      eq(cleaningOperationsRecordsTable.kind, kind),
      eq(cleaningOperationsRecordsTable.reference, reference),
    )).get();
  const serialized = JSON.stringify(payload);
  if (existing) {
    db.update(cleaningOperationsRecordsTable).set({
      status,
      payload: serialized,
      updatedAt: new Date().toISOString(),
    }).where(eq(cleaningOperationsRecordsTable.id, existing.id)).run();
    return existing.id;
  }
  return db.insert(cleaningOperationsRecordsTable).values({
    kind,
    status,
    reference,
    payload: serialized,
...
const containerIds = containers.map(([reference, size, status, location, notes], index) => ({
  ...
  id: upsertRecord("container", reference, status, {
    ...
    typeName: `باقة التنظيف ${size} ياردة`,
    size: `${size} ياردة`,
