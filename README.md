# منصة خدمات التنظيف — CleanFlow

منصة تشغيل وتسويق عربية لخدمات التنظيف في الرياض. تجمع بين الموقع العام ولوحة إدارة الطلبات والتشغيل والباقات والعقود والتحصيل والتقارير.

## ما الذي يقدمه النظام؟

- استقبال طلبات خدمات التنظيف وتأجير الباقات التنظيف وتتبعها.
- إدارة العملاء والمواقع والأصول والباقات التنظيف والعقود والإيجارات.
...
- هيرو ونافبار متجاوبان مع إدارة شرائح الهيرو ورفع صورها وضغطها تلقائيًا إلى WebP؛ التفاصيل في [دليل الهيرو والنافبار وصور الشرائح](docs/hero-navbar-slide-management.md).
...
`/`، `/services`، `/packages`، `/packages`، `/areas`، `/blog`، `/contact`، `/faq`، `/chat`، `/track-order/:id`، `/page/:slug`.

### الإدارة

`/admin/login`، `/admin`، `/admin/requests`، `/admin/work-orders`، `/admin/conversations`، `/admin/notifications`، `/admin/employees`، `/admin/analytics`، `/admin/cleaning-operations`، `/admin/cleaning-operations/profile/customer/:id`، `/admin/cleaning-operations/profile/employee/:id`، `/admin/cleaning-operations/profile/container/:id`، `/admin/cleaning-operations/contract/:id/print`.
...
الصلاحية الظاهرة في القائمة ليست حماية بحد ذاتها؛ التحقق النهائي يجب أن يتم في API. مرجع نظام الباقات التنظيف الأمني هو:
...
مع الرفض المسبب عند الحاجة. لا تعدّل حركة الباقة التنظيف القديمة؛ التصحيح يتم بحركة جديدة موثقة.
...
- [دليل تشغيل وأمان نظام الباقات التنظيف](docs/container-system-security-and-operations.md)
- [خطة تطوير نظام الباقات التنظيف](docs/container-system-development-plan.md)
- [خطة صيانة نظام الباقات التنظيف](docs/container-system-maintenance-plan.md)
