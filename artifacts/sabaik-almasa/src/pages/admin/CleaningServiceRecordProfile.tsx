...
import { getGetCleaningOperationsQueryKey, getGetServiceRequestsQueryKey, useCreateCleaningContractWorkflow, useCreateCleaningOperationsRecord, useGetCleaningOperations, useGetServiceRequests } from "@workspace/api-client-react"
import type { CleaningOperationsRecord } from "@workspace/api-client-react"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { amountOf, FIELD_CONFIG, formatRecordDate, formatStatus, KIND_LABELS, RecordDialog, RecordStatus, RecordKind } from "./CleaningOperationsComponents"
...
type ProfileMode = "customer" | "employee" | "package"

const money = (value: number) => `${value.toLocaleString("ar-SA")} ر.س`
const payloadOf = (record?: CleaningOperationsRecord | null) => (record?.payload ?? {}) as Record<string, unknown>
...
const collectionKey = (record: CleaningOperationsRecord) => {
...
const canonicalCollections = (records: CleaningOperationsRecord[]) => {
...
const allocatedAmount = (record: CleaningOperationsRecord, key: "contractId" | "invoiceId", id: number) => {
...
const collectionAmountForInvoice = (record: CleaningOperationsRecord, invoice: CleaningOperationsRecord) => {
  const payload = payloadOf(record)
...
const customerSiteBelongsTo = (site: CleaningOperationsRecord, customer: CleaningOperationsRecord) => {
...
const siteAddress = (site: CleaningOperationsRecord) => {
...
function OperationalQuickActions({ customerId, customerName, phone, email, location, contracts, containers, onRefresh }: {
  ...
  contracts: CleaningOperationsRecord[]; containers: CleaningOperationsRecord[]; onRefresh: () => void
}) {
  ...
  const container = containers.find(item => item.status !== "archived" && (
    String(contractPayload.packageRecordId ?? "") === String(item.id) ||
  ...
  const run = async (kind: "pickup" | "empty") => {
    ...
    try {
      await createProfileWorkOrder({
        customerRecordId: customerId, packageRecordId: container.id, contractRecordId: contract.id,
        clientName: customerName, phone, email,
        serviceType: kind === "pickup" ? "استرجاع وسحب باقة التنظيف" : "أمر تفريغ باقة التنظيف",
        packageSize: text(ap.assetCode ?? ap.containerCode ?? container.reference),
        location, notes: kind === "pickup" ? "طلب استرجاع الباقة التنظيف من ملف العميل" : "طلب تفريغ الباقة التنظيف من ملف العميل",
      })
  ...
  return <Card className="border-amber-200 bg-amber-50/60 shadow-sm"><CardContent className="flex flex-wrap items-center justify-between gap-3 p-4"><div><p className="text-sm font-black text-amber-950">إجراءات تشغيل سريعة</p><p className="mt-1 text-xs text-amber-900/70">مرتبطة بالعقد والباقة التنظيف ولا تغيّر القيود المالية.</p></div><div className="flex flex-wrap gap-2"><Button size="sm" disabled={!!busy || !contract || !container || !customerId} onClick={() => run("pickup")} className="gap-2 bg-amber-700 hover:bg-amber-800"><RotateCcw size={14} />{busy === "pickup" ? "جارٍ الإنشاء..." : "استرجاع وسحب الباقة التنظيف"}</Button><Button size="sm" variant="outline" disabled={!!busy || !contract || !container || !customerId} onClick={() => run("empty")} className="gap-2 border-amber-300 text-amber-900 hover:bg-white"><Trash2 size={14} />{busy === "empty" ? "جارٍ الإنشاء..." : "أمر تفريغ"}</Button>{contract && <Button size="sm" variant="outline" onClick={() => navigate(`/admin/cleaning-operations?view=overview&contractId=${contract.id}&prepare=1`)} className="gap-2 border-cyan-300 text-cyan-900 hover:bg-white"><LockKeyhole size={14} /> تجهيز العقد</Button>}</div>{(!contract || !container) && <p className="w-full text-xs font-semibold text-amber-800">يلزم عقد نشط وباقة التنظيف مخصصة لإظهار إجراءات التشغيل.</p>}</CardContent></Card>
}
...
function FieldGrid({ record }: { record: CleaningOperationsRecord }) {
...
function RelatedRows({ title, records, empty = "لا توجد سجلات مرتبطة", headerAction }: { title: string; records: CleaningOperationsRecord[]; empty?: string; headerAction?: ReactNode }) {
  const embeddedAction = (records as CleaningOperationsRecord[] & { headerAction?: ReactNode }).headerAction
...
function InvoiceRows({ invoices, payments }: { invoices: CleaningOperationsRecord[]; payments: CleaningOperationsRecord[] }) {
  const [, navigate] = useLocation()
  return <Card className="border-cyan-100 shadow-sm"><CardHeader className="border-b border-slate-100 px-5 py-4"><div className="flex items-center justify-between gap-3"><CardTitle className="text-base">الفواتير</CardTitle><span className="rounded-full bg-cyan-50 px-2.5 py-1 text-xs font-black text-cyan-800">{invoices.length}</span></div></CardHeader><CardContent className="p-0">{invoices.length === 0 ? <p className="p-8 text-center text-sm text-slate-500">لا توجد فواتير مرتبطة بهذا العميل.</p> : invoices.map(invoice => {
    ...
    return <button type="button" key={invoice.id} onClick={() => navigate(`/admin/cleaning-operations/invoice/${invoice.id}/details`)} className="grid w-full grid-cols-2 gap-3 border-b border-slate-100 px-5 py-4 text-right transition hover:bg-cyan-50/40 sm:grid-cols-[1.15fr_1fr_1fr_1fr_1fr]">
      ...
      <span><small className="block text-[11px] text-slate-400">الباقة التنظيف</small><b className="text-xs" dir="ltr">#{text(p.containerCode)}</b></span>
...
function findProfileRecord(records: CleaningOperationsRecord[], mode: ProfileMode, id: string) {
  const numericId = Number(id)
  return records.find(record => record.id === numericId && record.kind === mode)
    ?? records.find(record => record.id === numericId && (mode === "employee" ? ["employee", "driver"].includes(record.kind) : mode === "package" ? ["package", "package_asset"].includes(record.kind) : record.kind === "customer"))
}

function CustomerProfile({ record, records }: { record: CleaningOperationsRecord; records: CleaningOperationsRecord[] }) {
  const [, navigate] = useLocation()
  ...
  const contractMutation = useCreateCleaningContractWorkflow()
  const closeAction = () => {
    setAction(null)
  ...
  const containers = records.filter(item => ["container", "container_asset"].includes(item.kind) && (
    assignments.some(assignment => String(payloadOf(assignment).packageRecordId) === String(item.id)) ||
  ...
  const refreshProfile = () => {
    void queryClient.invalidateQueries({ queryKey: getGetCleaningOperationsQueryKey() })
  }
  ...
  return <div className="space-y-5">
    ...
    />
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Stat label="إجمالي العقود" value={contracts.length} /><Stat label="المواقع" value={sites.length} /><Stat label="الباقات التنظيف الحالية" value={containers.length} /><Stat label="المواعيد القادمة" value={upcomingAppointments.length} /></div>
    ...
    <div className="grid gap-5 xl:grid-cols-2"><RelatedRows title="مواقع العميل" records={sites} empty="لم تتم إضافة موقع لهذا العميل بعد" /><RelatedRows title="الباقات التنظيف المخصصة" records={containers} empty="لا توجد باقات التنظيف مخصصة حاليًا" /></div>
...
function EmployeeProfile({ record, records }: { record: CleaningOperationsRecord; records: CleaningOperationsRecord[] }) {
  const p = payloadOf(record)
...
function CleaningPackageProfile({ record, records }: { record: CleaningOperationsRecord; records: CleaningOperationsRecord[] }) {
  const p = payloadOf(record)
  const code = text(p.assetCode ?? p.containerCode ?? p.code ?? record.reference)
  const currentStatus = p.status ?? record.status
  const related = records.filter(item => {
    const payload = payloadOf(item)
    return text(payload.containerCode ?? payload.assetCode, "") === code || text(payload.packageRecordId, "") === String(record.id)
  })
  ...
  const workOrdersQuery = useGetServiceRequests(undefined, { query: { queryKey: getGetServiceRequestsQueryKey(), staleTime: 30_000 } })
  const workOrders = (workOrdersQuery.data ?? []).filter(item => {
    const linked = item as typeof item & { packageRecordId?: number | null; contractRecordId?: number | null }
    return linked.packageRecordId === record.id || (linked.contractRecordId != null && contracts.some(contract => contract.id === linked.contractRecordId))
  })
  ...
  const customerPayload = payloadOf(customer)
  const timeline = [
    ...movements.map(item => ({ id: `movement-${item.id}`, date: item.createdAt, title: text(payloadOf(item).movementType, "حركة باقة التنظيف"), detail: text(payloadOf(item).location, "الموقع غير محدد"), icon: ArrowRight, tone: "bg-cyan-50 text-cyan-800" })),
  ...
  const workOrderRecords: CleaningOperationsRecord[] = workOrders.map(item => ({
    id: item.id,
    kind: "appointment",
  ...
  />
  ;(workOrderRecords as CleaningOperationsRecord[] & { headerAction?: ReactNode }).headerAction = quickActions
...
export function CleaningPackageRecordProfile({ mode }: { mode: ProfileMode }) {
  const [, setLocation] = useLocation()
  const navigate = (to: string) => to === "/admin/cleaning-operations" ? (window.history.length > 1 ? window.history.back() : setLocation(to)) : setLocation(to)
  const params = useParams<{ id: string }>()
  const query = useGetCleaningOperations()
  const records = query.data?.records ?? []
  const record = useMemo(() => findProfileRecord(records, mode, params.id), [mode, params.id, records])
  const p = payloadOf(record)
  const title = mode === "customer" ? text(p.name ?? p.customerName, "ملف العميل") : mode === "employee" ? text(p.name ?? p.employeeName ?? p.driverName, "ملف الموظف") : text(p.assetCode ?? p.packageCode ?? p.code, "ملف الباقة التنظيف")
  const Icon = mode === "customer" ? UserRound : mode === "employee" ? BriefcaseBusiness : Truck
  const goBack = () => window.history.length > 1 ? window.history.back() : navigate("/admin/cleaning-operations")
  ...
  if (!record) return <div className="space-y-4 rounded-2xl border border-rose-200 bg-rose-50 p-8 text-center"><h2 className="font-black text-rose-900">لم يتم العثور على الملف</h2><Button onClick={goBack} variant="outline">العودة إلى نظام الباقات التنظيف</Button></div>
  ...
  return <div dir="rtl" className="container-system space-y-5"><div className="flex flex-wrap items-center justify-between gap-3"><Button variant="ghost" onClick={() => navigate("/admin/cleaning-operations")} className="gap-2 px-0 text-cyan-800"><ArrowRight size={16} /> العودة لنظام الباقات التنظيف</Button><div className="flex gap-2"><Button variant="outline" onClick={() => window.print()} className="gap-2"><Printer size={15} /> طباعة الملف</Button><Button onClick={() => navigate(`/admin/cleaning-operations/profile/${mode}/${record.id}`)} className="hidden">الملف</Button></div></div><Card className="overflow-hidden border-0 bg-[#123d4e] text-white shadow-[0_14px_40px_rgba(18,61,78,.18)]"><CardContent className="p-4 sm:p-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-center">{mode === "package" ? <div className="order-2 flex h-28 w-full shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white sm:order-1 sm:h-28 sm:w-56"><CleaningServiceStatus status={p.status ?? record.status} code={title} className="h-full w-full" numberClassName="top-[45.5%]" /></div> : <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-amber-400 text-slate-900"><Icon size={30} /></div>}<div className="order-1 min-w-0 flex-1 sm:order-2"><p className="text-xs font-bold text-cyan-200">{mode === "customer" ? "ملف عميل" : mode === "employee" ? "ملف موظف" : "ملف أصل باقة التنظيف"}</p><h1 className="mt-1 text-2xl font-black" dir={mode === "package" ? "ltr" : undefined}>{title}</h1><div className="mt-3 flex flex-wrap gap-3 text-xs text-cyan-100/75">{mode !== "package" && <><span className="flex items-center gap-1"><Phone size={13} /> {text(p.phone ?? p.customerPhone, "لا يوجد هاتف")}</span><span className="flex items-center gap-1"><MapPin size={13} /> {text(headerAddress, "لا يوجد عنوان")}</span>{mode === "customer" && customerSites.length > 0 && <span className="flex items-center gap-1"><MapPin size={13} /> {customerSites.length} {customerSites.length === 1 ? "موقع مسجل" : "مواقع مسجلة"}</span>}</>} {mode === "package" && <span>{text(p.typeName ?? p.packageType, "نوع الباقة التنظيف غير محدد")} · {text(p.size ?? p.capacity, "الحجم غير محدد")}</span>}<span className="flex items-center gap-1"><CalendarDays size={13} /> آخر تحديث {formatRecordDate(record.updatedAt)}</span></div></div><RecordStatus status={String(p.status ?? record.status)} /></div></CardContent></Card>{mode !== "package" && <Card className="border-slate-200/80 shadow-sm"><CardHeader className="border-b border-slate-100 px-5 py-4"><CardTitle className="text-base">البيانات الأساسية</CardTitle></CardHeader><CardContent className="p-5"><FieldGrid record={record} /></CardContent></Card>}{mode === "customer" ? <CustomerProfile record={record} records={records} /> : mode === "employee" ? <EmployeeProfile record={record} records={records} /> : <CleaningPackageProfile record={record} records={records} />}</div>
}

export function ContractPrintPage() {
  const [, setLocation] = useLocation()
  const navigate = (to: string) => to === "/admin/cleaning-operations" ? (window.history.length > 1 ? window.history.back() : setLocation("/admin/cleaning-operations?view=contracts_list")) : setLocation(to)
  const params = useParams<{ id: string }>()
  const query = useGetCleaningOperations()
  ...
  if (!record) return <div className="p-10 text-center"><p className="mb-4 font-bold">العقد غير موجود</p><Button onClick={() => navigate("/admin/cleaning-operations")}>العودة</Button></div>
