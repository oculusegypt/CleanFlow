...
import {
  ...
  Menu, X, BarChart3, BookOpen, Users, UserCircle,
...
const ALL_NAV = [
  ...
  { href: "/admin/cleaning-operations",  icon: Truck,             label: "عمليات الباقات التنظيف", group: "main",    section: "container_system" },
  { href: "/admin/work-orders",    icon: Truck,             label: "مهام السائقين",      group: "main",    section: "work_orders" },
  { href: "/admin/requests",       icon: Inbox,             label: "الطلبات",          group: "operations", section: "requests" },
  { href: "/admin/services",       icon: Settings,          label: "الخدمات",          group: "operations", section: "services" },
  { href: "/admin/packages",       icon: Box,               label: "كتالوج الباقات", group: "operations", section: "packages" },
  { href: "/admin/offers",         icon: Sparkles,           label: "العروض الموسمية",  group: "operations", section: "ads" },
  { href: "/admin/conversations",  icon: MessageSquare,     label: "المحادثات",        group: "operations", section: "conversations" },
  { href: "/admin/whatsapp",       icon: MessageCircle,     label: "واتساب",           group: "operations", section: "whatsapp" },
  { href: "/admin/notifications",  icon: Bell,              label: "الإشعارات",        group: "operations", section: "notifications" },
  { href: "/admin/analytics",      icon: BarChart3,         label: "التقارير والتحليلات", group: "insights", section: "analytics" },
  { href: "/admin/blog",           icon: BookOpen,          label: "المدونة",          group: "insights", section: "blog" },
  { href: "/admin/seo-pages",      icon: FilePenLine,       label: "الصفحات",          group: "insights", section: "seo_pages" },
  { href: "/admin/seo",            icon: Search,             label: "SEO",              group: "insights", section: "seo" },
...
const GROUPS: { key: string; label: string }[] = [
  ...
  { key: "insights", label: "المحتوى والتحليلات" },
  { key: "content", label: "المحتوى الإضافي" },
...
const ROLE_INFO: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  ...
  customer_service: { label: "خدمة عملاء",  icon: Headphones,    color: "bg-green-500"  },
  requests_officer: { label: "مسؤول طلبات", icon: ClipboardList, color: "bg-amber-500"  },
...
function SidebarContent({ permissions = [], onNavClick, onLogout, userName, userRole, conversationCount = 0, pendingRequests = 0, unreadNotifications = 0 }: {
  ...
  const visibleNav = userRole === "driver"
    ...
    : ["admin", "manager"].includes(userRole)
      ...
      : ALL_NAV.filter(item => item.section !== "work_orders" && hasPerm(item.section));
  const roleInfo = ROLE_INFO[userRole] ?? ROLE_INFO.admin
  const RIcon = roleInfo.icon
  return (
    <div className="flex min-h-0 flex-col h-full">
       <div className="p-5 border-b border-white/10 shrink-0">
         {isLoaded && logoUrl ? <img src={logoUrl} alt={`شعار ${companyName}`} className="h-14 max-w-full w-auto object-contain" /> : <div className="h-14" aria-hidden="true" />}
      </div>

      <nav className="admin-sidebar-scrollbar-hidden min-h-0 flex-1 overflow-y-auto py-3 overscroll-contain" aria-label="قائمة إدارة النظام">
        {GROUPS.map(group => {
...
function AdminHeaderLogo() {
  const { companyName, logoUrl, isLoaded } = useSiteSettings()
  return isLoaded && logoUrl ? <img src={logoUrl} alt={`شعار ${companyName}`} className="sm:hidden h-7 w-auto object-contain" /> : <span className="sm:hidden h-7 w-20" aria-hidden="true" />
}
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/auth/me`, {
      ...
      .then((data: { role: string; permissions: string[]; name: string; id: number } | null) => {
        if (!data) return
  ...
  useEffect(() => {
    ...
    if (!authLoading && !isDriver && !isWorkOrdersManager && location === "/admin/work-orders") {
      setLocation("/admin")
    }
  }, [authLoading, isDriver, isWorkOrdersManager, driverAllowedRoute, location, setLocation])
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-primary flex items-center justify-center" dir="rtl">
  ...
  if ((isDriver && !driverAllowedRoute) || (!isDriver && !isWorkOrdersManager && location === "/admin/work-orders")) {
    return (
      <div className="min-h-screen bg-primary flex items-center justify-center" dir="rtl">
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 border-4 border-white/20 border-t-white rounded-full animate-spin" />
  ...
  return (
    <div className="admin-shell min-h-screen bg-gray-100 flex min-w-0 w-full max-w-full font-sans overflow-x-hidden" dir="rtl">
      ...
      {/* Mobile Overlay */}
      {drawerOpen && (
        <div className="fixed inset-0 bg-black/50 z-30 lg:hidden" onClick={() => setDrawerOpen(false)} />
      )}

      {/* Mobile Drawer */}
      <aside className={`fixed inset-y-0 right-0 z-40 flex h-dvh min-h-0 w-[min(18rem,calc(100vw-1rem))] max-w-[calc(100vw-1rem)] flex-col overflow-hidden bg-primary text-white transform transition-transform duration-300 ease-in-out lg:hidden ${drawerOpen ? "translate-x-0" : "translate-x-full"}`}>
      ...
      <main className="flex min-h-screen min-w-0 w-full max-w-full flex-1 flex-col overflow-x-hidden lg:mr-60 xl:mr-64">
        <header className="flex h-14 min-w-0 w-full shrink-0 items-center justify-between overflow-visible border-b border-gray-100 bg-white px-3 shadow-sm sticky top-0 z-10 sm:h-16 sm:px-6">
          <button
            ...
            onClick={() => setDrawerOpen(true)} aria-label="فتح القائمة">
            <Menu size={20} />
          </button>
            ...
            {!isDriver && (
              <>
                <Link href="/admin/profile"
                  ...
                  <div className={`w-7 h-7 ${roleInfo.color} rounded-full flex items-center justify-center text-white text-xs font-bold`}>
                    {initials}
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-700 font-medium leading-none">{userName || "مدير"}</p>
