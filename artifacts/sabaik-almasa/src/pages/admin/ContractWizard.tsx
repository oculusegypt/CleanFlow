...
const DEFAULT_CLAUSES = [
  "يلتزم الطرف الأول بتوفير الباقة التنظيف وتسليمها إلى الموقع المحدد في العقد، وتنفيذ خدمات النقل والتفريغ المتفق عليها.",
  "يلتزم الطرف الثاني بالمحافظة على الباقة التنظيف وعدم نقلها أو استخدامها لغير الغرض المتفق عليه دون موافقة الطرف الأول.",
...
function payloadOf(record: CleaningOperationsRecord) {
  return record.payload as Record<string, unknown>
}

function labelOf(record: CleaningOperationsRecord) {
...
export function ContractWizard({ open, records, initialCustomerId = null, initialRequest = null, busy = false, onClose, onSubmit }: ContractWizardProps) {
  ...
  useEffect(() => {
    ...
    const requestSite = initialRequest
      ...
      : undefined
    const requestCleaningPackage = initialRequest
      ? records.find(record => {
          ...
          return ["available", "متاح", "active"].includes(record.status) &&
            Boolean(initialRequest.packageSize) && (type.includes(initialRequest.packageSize) || initialRequest.packageSize.includes(type))
        })
      : undefined
    ...
    setForm({
      ...
      packageRecordId: requestCleaningPackage?.id ? String(requestCleaningPackage.id) : "",
      ...
      packageSize: initialRequest?.packageSize ?? "",
  ...
  const container = containers.find(record => String(record.id) === form.packageRecordId)
  ...
  const packageSize = String(containerPayload?.size ?? containerPayload?.packageSize ?? containerPayload?.capacity ?? containerPayload?.typeName ?? "")
  ...
  const selectCleaningPackage = (value: string) => {
    ...
    const selectedSize = String(selectedPayload.size ?? selectedPayload.packageSize ?? selectedPayload.capacity ?? selectedPayload.typeName ?? "")
    setError("")
    setForm(current => ({
      ...current,
      packageRecordId: value,
        ...
        <CardContent className="p-0">
          ...
          <div className="space-y-5 p-5 sm:p-7">
            {step === 0 && <section className="space-y-4">
              ...
              {initialRequest && <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-xs leading-6 text-indigo-950"><div className="font-black">تم تحميل بيانات الطلب #{initialRequest.id}</div><div className="mt-1">الاسم: <b>{initialRequest.clientName}</b> · الجوال: <b dir="ltr">{initialRequest.phone}</b></div><div>الخدمة: <b>{initialRequest.serviceType}</b> · نوع الباقة التنظيف: <b>{initialRequest.packageSize || "غير محدد"}</b></div><div>الموقع: <b>{initialRequest.location}</b></div></div>}
            ...
            {step === 1 && <section className="space-y-4">
               ...
               {initialRequest && <div className="rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4 text-xs leading-6 text-indigo-950"><div className="mb-2 font-black">بيانات الطلب المحملة تلقائياً</div><div className="grid gap-x-4 sm:grid-cols-2"><span>العميل: <b>{initialRequest.clientName}</b></span><span>الجوال: <b dir="ltr">{initialRequest.phone}</b></span><span>الخدمة: <b>{initialRequest.serviceType}</b></span><span>الباقة التنظيف المطلوبة: <b>{initialRequest.packageSize || "غير محدد"}</b></span><span className="sm:col-span-2">العنوان: <b>{initialRequest.location}</b></span>{initialRequest.duration && <span>المدة: <b>{initialRequest.duration}</b></span>}{initialRequest.notes && <span className="sm:col-span-2">الملاحظات: <b>{initialRequest.notes}</b></span>}</div></div>}
               <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div><Label htmlFor="contract-number">رقم العقد</Label><Input id="contract-number" value={form.contractNumber} onChange={event => update("contractNumber", event.target.value)} placeholder="سيولد تلقائياً عند الحفظ" className="mt-2" dir="ltr" /><p className="mt-1 text-[11px] text-slate-500">اتركه فارغاً ليولد النظام رقماً فريداً تلقائياً.</p></div>
                 <div><Label htmlFor="contract-container">أصل الباقة التنظيف</Label><select id="contract-container" value={form.packageRecordId} onChange={event => selectCleaningPackage(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm"><option value="">اختر الأصل</option>{containers.map(record => <option key={record.id} value={record.id}>{labelOf(record)} · {String(payloadOf(record).typeName ?? payloadOf(record).size ?? "نوع غير محدد")}</option>)}</select></div>
                  ...
                  <div><Label htmlFor="contract-size">حجم الباقة التنظيف</Label><Input id="contract-size" value={form.packageSize || packageSize} readOnly placeholder="يُحدد تلقائيًا من الأصل" className="mt-2 bg-slate-50" /></div>
            ...
            {step === 3 && <section className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <div><Label htmlFor="appointment-type">نوع العملية</Label><select id="appointment-type" value={form.appointmentType} onChange={event => update("appointmentType", event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm"><option value="delivery">تسليم الباقة التنظيف</option><option value="pickup">استرجاع الباقة التنظيف</option><option value="inspection">فحص وتجهيز</option></select></div>
              ...
              {step === 4 && <section className="space-y-4"><div><h3 className="font-black text-slate-900">مراجعة قبل الإضافة</h3><p className="mt-1 text-sm text-slate-500">اضغط «تأكيد وإضافة العقد» لحفظ التعاقد وبنوده.</p></div>{initialRequest && <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-xs leading-6 text-indigo-950"><b>مصدر العقد: طلب الخدمة #{initialRequest.id}</b><div>العنوان: {initialRequest.location} · الباقة التنظيف المطلوبة: {initialRequest.packageSize || "غير محدد"}</div></div>}<div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm sm:grid-cols-2"><span>العميل: <b>{customer ? labelOf(customer) : initialRequest?.clientName || "—"}</b></span><span>الموقع: <b>{site ? labelOf(site) : initialRequest?.location || "—"}</b></span><span>نوع العقد: <b>{form.contractType}</b></span><span>رقم القطعة / المخطط: <b>{form.propertyNumber || "—"} / {form.planNumber || "—"}</b></span><span>الأصل: <b>{container ? labelOf(container) : initialRequest?.packageSize || "—"}</b></span><span>الفترة: <b dir="ltr">{form.startDate || "—"} → {form.endDate || "—"}</b></span><span>عدد البنود: <b>{form.clauses.length}</b></span><span>الإجمالي: <b>{total.toLocaleString("ar-SA")} ر.س</b></span></div><div className="flex items-start gap-2 rounded-xl border border-amber-100 bg-amber-50 p-3 text-xs leading-6 text-amber-950"><ShieldCheck size={16} className="mt-1 shrink-0" />سيُحفظ العقد مع بيانات العميل والموقع والأصل والبنود المضافة، وسيُنشأ موعد مرتبط به.</div></section>}
