...
import { getGetFinancialTruthQueryKey, useGetFinancialTruth, type CleaningOperationsRecord } from "@workspace/api-client-react"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import type { RecordKind } from "./CleaningOperationsComponents"
...
function payload(record: CleaningOperationsRecord) {
  return record.payload as Record<string, unknown>
}

function amount(record: CleaningOperationsRecord) {
  return Number(payload(record).amount ?? payload(record).total ?? payload(record).value ?? 0) || 0
}

function collectionKey(record: CleaningOperationsRecord) {
...
function RecordTable({ records, empty }: { records: CleaningOperationsRecord[]; empty: string }) {
