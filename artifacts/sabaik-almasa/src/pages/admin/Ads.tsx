...
const POSITIONS = [
  ...
  { value: "after_offers",        label: "قسم العروض",          desc: "أرشيف العروض الموسمية في الموقع" },
  ...
  { value: "after_services",      label: "بعد الخدمات",        desc: "بين الخدمات والباقات التنظيف" },
  { value: "after_containers",    label: "بعد الباقات التنظيف",       desc: "بين الباقات التنظيف وطريقة العمل" },
...
export default function AdminAds() {
  ...
  const offersMode = location === "/admin/offers"
  ...
  const openNew = () => { setForm({ ...emptyForm(), position: offersMode ? "after_offers" : "after_hero", type: offersMode ? "offer" : "banner", order: ads.length }); setUploadError(""); setEditing("new") }
  const openEdit = (a: Ad) => {
  ...
  const pickAndUpload = () => {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        fd.append("file", file)
        fd.append("title", form.title || "إعلان خدمات التنظيف بالرياض")
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">{offersMode ? "إدارة العروض الموسمية" : "الإعلانات"}</h2>
          <p className="text-sm text-gray-500 mt-0.5">{offersMode ? "أنشئ عروضًا موسمية تظهر في الموقع وتبقى قابلة للأرشفة عبر صفحة العروض." : "أضف إعلانات تظهر في مواضع محددة من الصفحة"}</p>
        </div>
      ...
      <div className="space-y-3">
        {POSITIONS.filter(pos => !offersMode || pos.value === "after_offers").map(pos => {
          const group = ads.filter(a => a.position === pos.value && (!offersMode || a.type === "offer"))
          return (
            <div key={pos.value}>
