...
function GeneralTab() {
  ...
  async function uploadLogo(file: File) {
    ...
    try {
      ...
      form.append("title", "شعار السهم كلين وخدمات التنظيف بالرياض")
      ...
      <Card className="border-2 border-slate-100 shadow-sm overflow-hidden">
        <CardHeader className="bg-slate-50/50 pb-3 border-b border-slate-100">
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-lg font-black text-slate-900">
              ...
              ألوان وهوية الموقع (ثيم الباقات التنظيف والخدمات)
            </CardTitle>
        ...
        <CardContent className="p-6 space-y-4">
          <p className="text-sm text-slate-600 leading-relaxed">
            اختر لوحة الألوان المناسبة لطبيعة نشاطك وهوية منشأتك. تم تصميم كل لوحة بعناية لتلائم قطاع المقاولات، الباقات التنظيف، وإدارة المخلفات.
          </p>
...
function SlidesTab() {
  ...
  const openEdit = (s: HeroSlide) => {
    setForm({ title: s.title, subtitle: s.subtitle, imageUrl: s.imageUrl, ctaText: s.ctaText ?? "", order: s.order, isActive: s.isActive })
    setUploadError("")
    setEditing(s.id)
  }
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      setForm(current => ({
        ...current,
        imageUrl: result.url!.startsWith("http") ? result.url! : `${API_BASE}${result.url}`,
      }))
          ...
          <CardContent className="p-6 space-y-4">
            ...
            <div><label className="text-sm font-medium text-gray-700 mb-1 block">النص التوضيحي</label><Input value={form.subtitle} onChange={e => setForm({ ...form, subtitle: e.target.value })} placeholder="وصف مختصر" dir="rtl" /></div>
             <div>
               ...
               <div className="space-y-3">
                 <label
                   htmlFor="site-settings-hero-image-upload"
                 ...
                 >
                   ...
                   <input
                     id="site-settings-hero-image-upload"
                     ref={fileInputRef}
                     type="file"
            ...
            <div className="flex gap-3 pt-2">
              <Button onClick={handleSave} disabled={creating || updating || uploading} className="gap-2"><Check size={16} />{creating || updating ? "جاري الحفظ..." : "حفظ"}</Button>
              <Button variant="outline" onClick={() => setEditing(null)} disabled={uploading} className="gap-2"><X size={16} />إلغاء</Button>
            </div>
          </CardContent>
        </Card>
      )}
...
const SECTION_DEFS = [
  ...
  { id: "packages",        label: "باقات التنظيف الأنقاض والنفايات", description: "باقات التنظيف الأنقاض والنفايات والمكابس المتاحة" },
