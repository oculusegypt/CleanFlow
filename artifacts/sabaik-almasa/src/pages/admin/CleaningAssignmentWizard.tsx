...
type Props = {
  ...
  initialCleaningPackageId?: number | null
  busy?: boolean
  onClose: () => void
...
const labelOf = (record: CleaningOperationsRecord) => String(payloadOf(record).name ?? payloadOf(record).customerName ?? payloadOf(record).contractNumber ?? payloadOf(record).assetCode ?? record.reference ?? `#${record.id}`)

export function CleaningPackageAssignmentWizard({ open, records, initialCleaningPackageId = null, busy = false, onClose, onSubmit }: Props) {
  const [contractRecordId, setContractRecordId] = useState("")
  const [packageRecordId, setCleaningPackageRecordId] = useState("")
  ...
  useEffect(() => {
    ...
    setContractRecordId("")
    setSiteRecordId("")
    setCleaningPackageRecordId(initialCleaningPackageId ? String(initialCleaningPackageId) : "")
    setError("")
  }, [initialCleaningPackageId, open])

  if (!open) return null
  const submit = () => {
    if (!contract || !siteRecordId || !packageRecordId) {
      setError("اختر عقداً وموقعاً تابعاً له وأصلاً متاحاً")
      return
    }
    onSubmit({ contractRecordId: contract.id, siteRecordId: Number(siteRecordId), packageRecordId: Number(packageRecordId), assignmentStatus: "reserved", startDate: new Date().toISOString().slice(0, 10), notes: "تم التخصيص من المسار السياقي" })
  }
    ...
    <Card className="w-full max-w-2xl overflow-hidden border-cyan-100 shadow-2xl">
      <CardContent className="p-0">
        <header className="flex items-start justify-between bg-[#123d4e] p-6 text-white">
          <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-amber-400 text-slate-900"><Link2 size={20} /></div><div><h2 className="text-xl font-black">تخصيص أصل لباقة التنظيف</h2><p className="mt-1 text-xs text-cyan-100/75">اختر العلاقات من سياقها؛ لا حاجة لإدخال المعرّفات يدوياً.</p></div></div>
        ...
        <div className="space-y-5 p-6">
          ...
          <div><Label htmlFor="assignment-container">أصل الباقة التنظيف المتاح</Label><select id="assignment-container" value={packageRecordId} onChange={event => { setCleaningPackageRecordId(event.target.value); setError("") }} className="mt-2 h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm"><option value="">اختر الأصل</option>{containers.map(record => <option key={record.id} value={record.id}>{labelOf(record)} · {String(payloadOf(record).typeName ?? "نوع غير محدد")}</option>)}</select></div>
