...
import { getGetAdminWorkOrdersQueryKey, useGetAdminStats, useGetAdminWorkOrders, useGetCleaningOperations } from "@workspace/api-client-react"
...
function OperationsExceptions() {
  ...
  const containerQuery = useGetCleaningOperations({
    query: { queryKey: ["/api/admin/cleaning-operations"], staleTime: 30_000 },
  })
  ...
  const items = [
    ...
    { label: "عقود منتهية", value: expiredContracts.length, href: "/admin/cleaning-operations", tone: "border-indigo-200 bg-indigo-50 text-indigo-700", icon: CalendarClock },
  ]
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        >
          ...
          <div className="relative grid grid-cols-2 gap-2.5 sm:grid-cols-3">
            {[
              { href: "/admin/cleaning-operations?view=customer&create=1", icon: UserPlus, label: "عميل جديد", hint: "إضافة ملف عميل", tone: "blue" },
              { href: "/admin/cleaning-operations?view=invoice&create=1", icon: ReceiptText, label: "فاتورة جديدة", hint: "إنشاء فاتورة", tone: "cyan" },
              { href: "/admin/cleaning-operations?view=contract", icon: FilePlus2, label: "عقد جديد", hint: "بدء عقد وباقة التنظيف", tone: "violet" },
              { href: "/admin/employees", icon: Users, label: "الموظفون", hint: "إدارة الفريق", tone: "emerald" },
              { href: "/admin/cleaning-operations?view=receipt&create=1", icon: Landmark, label: "السندات", hint: "قبض وصرف", tone: "amber" },
              { href: "/admin/cleaning-operations?view=payment&create=1", icon: HandCoins, label: "سداد العملاء", hint: "تسجيل دفعة", tone: "green" },
              { href: "/admin/requests", icon: ClipboardPlus, label: "طلب جديد", hint: "متابعة الطلبات", tone: "rose" },
              { href: "/admin/cleaning-operations?view=container&create=1", icon: Truck, label: "الباقات التنظيف", hint: "الأصول والتشغيل", tone: "slate" },
