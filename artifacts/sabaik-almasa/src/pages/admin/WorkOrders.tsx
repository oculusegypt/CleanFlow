...
import { SignaturePad } from "./CleaningOperationsComponents";
import {
  Check,
  CheckCircle2,
  ClipboardList,
          ...
          <section className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Info
                ...
                value={order.packageSize || "غير محدد"}
              />
...
export default function WorkOrders() {
  ...
  function handleAction(order: ServiceRequest, status: DriverWorkOrderStatus) {
    ...
    const isCleaningPackageOrder =
      Boolean(extended.contractRecordId || extended.packageRecordId) ||
      /حاوي|أنقاض|تفريغ|سحب|استرجاع|تسليم|container|debris|waste/i.test(
        `${order.serviceType} ${order.packageSize}`,
      );
    if (status === DriverWorkOrderStatus.completed && isCleaningPackageOrder) {
