...
import { useGetCleaningOperations, type CleaningOperationsRecord } from "@workspace/api-client-react"
...
const payloadOf = (record?: CleaningOperationsRecord) => (record?.payload ?? {}) as Record<string, unknown>
...
const allocatedAmount = (record: CleaningOperationsRecord, invoiceId: number) => {
...
const findInvoiceContract = (records: CleaningOperationsRecord[], invoice: CleaningOperationsRecord, customer?: CleaningOperationsRecord, container?: CleaningOperationsRecord) => {
...
export default function InvoicePrintPage() {
  ...
  const container = records.find(item => ["container", "container_asset"].includes(item.kind) && item.id === Number(p.packageRecordId))
  ...
  const containerId = container?.id ?? Number(p.packageRecordId)
  ...
  return <div dir="rtl" className="invoice-details-view min-h-screen bg-slate-100 p-4 sm:p-8">
    ...
    <div className="screen-only mx-auto mb-4 flex max-w-6xl flex-wrap items-center justify-between gap-3">
      <Button variant="ghost" onClick={() => window.history.length > 1 ? window.history.back() : navigate("/admin/cleaning-operations?view=invoice")} className="gap-2"><ArrowRight size={16} /> العودة إلى الفواتير</Button>
     ...
     <main className="screen-only mx-auto max-w-6xl space-y-5">
      <Card className="overflow-hidden border-cyan-100 shadow-sm">
        <CardContent className="bg-gradient-to-l from-cyan-950 to-cyan-800 p-6 text-white">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div><p className="text-sm text-cyan-100">تفاصيل الفاتورة</p><h1 className="mt-2 text-3xl font-black" dir="ltr">{invoiceNumber}</h1><p className="mt-2 text-sm text-cyan-100">فاتورة خدمات باقة التنظيف مرتبطة بعقد</p></div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
        <Card className="border-slate-200 shadow-sm"><CardHeader><CardTitle>الأطراف والأصل المفوتر</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">
          <RelatedLink href={customerId ? `/admin/cleaning-operations/profile/customer/${customerId}` : undefined} label="العميل"><UserRound size={14} />{value(p.customerName ?? payloadOf(customer).name)}</RelatedLink>
           <RelatedLink href={contractId ? `/admin/cleaning-operations/contract/${contractId}/print` : undefined} label="العقد"><FileDown size={14} />{contractNumber}</RelatedLink>
           <RelatedLink href={containerId ? `/admin/cleaning-operations/profile/container/${containerId}` : undefined} label="الباقة التنظيف"><span dir="ltr">#{containerCode}</span></RelatedLink>
           <RelatedLink href={customerId ? `/admin/cleaning-operations/profile/customer/${customerId}` : undefined} label="الموقع"><MapPin size={14} />{value(p.serviceAddress ?? p.location ?? payloadOf(site).address)}</RelatedLink>
