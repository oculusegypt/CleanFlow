...
import { useCreateCleaningOperationsRecord, type CleaningOperationsRecord, type ServiceRequest } from "@workspace/api-client-react"
...
export const KIND_LABELS: Record<RecordKind, string> = {
  ...
  container_type: "أنواع الباقات التنظيف",
  container: "الباقات التنظيف",
  container_assignment: "تخصيص الباقات التنظيف",
  contract: "العقود",
...
export const FIELD_CONFIG: Record<RecordKind, FieldConfig[]> = {
  ...
  container_type: [
    { key: "name", label: "اسم النوع", placeholder: "باقة التنظيف أنقاض 20 ياردة" },
  ...
  container: [
    ...
    { key: "typeName", label: "نوع الباقة التنظيف", placeholder: "20 ياردة" },
    { key: "status", label: "الحالة التشغيلية لأصل الباقة التنظيف", placeholder: "اختر الحالة التي تصف وضع الباقة التنظيف الآن" },
  ...
  container_assignment: [
    ...
    { key: "packageRecordId", label: "معرّف أصل الباقة التنظيف", placeholder: "الباقة التنظيف الفعلية" },
  ...
  contract: [
    ...
    { key: "packageRecordId", label: "معرّف أصل الباقة التنظيف", placeholder: "اختياري" },
    ...
    { key: "containerCode", label: "رقم الباقة التنظيف", placeholder: "CNT-204" },
  ...
  contract_line: [
    ...
    { key: "containerCode", label: "رقم الباقة التنظيف", placeholder: "CNT-101" },
  ...
  expense: [
    ...
    { key: "expenseType", label: "نوع المصروف", placeholder: "وقود / صيانة باقة التنظيف / تعويض موظف" },
    ...
    { key: "packageRecordId", label: "الباقة التنظيف (اختياري)", placeholder: "رقم أصل الباقة التنظيف" },
  ...
  appointment: [
    ...
    { key: "containerCode", label: "الباقة التنظيف", placeholder: "رقم الباقة التنظيف" },
    { key: "driverName", label: "السائق", placeholder: "اسم السائق" },
    { key: "vehiclePlate", label: "المركبة", placeholder: "لوحة المركبة" },
  ...
  invoice: [
    ...
    { key: "description", label: "وصف الخدمة أو البند", placeholder: "مثال: تأجير باقة التنظيف أنقاض 20 ياردة", wide: true, required: true },
    { key: "quantity", label: "الكمية", type: "number", placeholder: "1" },
...
export function amountOf(record: CleaningOperationsRecord) {
...
export const CONTRACT_TEMPLATES = [
  { id: "standard", name: "عقد إيجار باقة التنظيف قياسي", terms: "يلتزم الطرف الأول بتوفير الباقة التنظيف ونقلها إلى الموقع المحدد، ويلتزم الطرف الثاني بالمحافظة عليها وسداد القيمة في المواعيد المتفق عليها." },
  ...
  { id: "project", name: "عقد مشروع وموقع", terms: "يخص هذا العقد موقعًا أو مشروعًا محددًا، ويشمل مواقع الباقات التنظيف وجدول التبديل والتفريغ ومسؤوليات المشرف والممثل المعتمد." },
] as const
...
function InvoiceComposer({
  ...
  const contractLines = (contract: CleaningOperationsRecord): InvoiceLine[] => {
    ...
    const service = String(value.serviceType ?? value.service ?? value.rentType ?? "خدمة الباقة التنظيف")
    return [{
  ...
  const selectRequest = (value: string) => {
    ...
    const line: InvoiceLine = {
      ...
      containerType: request.packageSize ?? "",
      quantity: "1",
...
export function RecordDialog({
  ...
  open: boolean
  kind: RecordKind
  record?: CleaningOperationsRecord | null
  records?: CleaningOperationsRecord[]
  serviceRequests?: ServiceRequest[]
  ...
  const [createdCustomers, setCreatedCustomers] = useState<CleaningOperationsRecord[]>([])
  const [newCustomerOpen, setNewCustomerOpen] = useState(false)
  const [newCustomer, setNewCustomer] = useState({ name: "", phone: "", email: "" })
  const [newCustomerError, setNewCustomerError] = useState("")
  const createCustomerMutation = useCreateCleaningOperationsRecord()
  ...
  const remainingForContract = (item: CleaningOperationsRecord) => {
  ...
  const selectCustomer = (customer: CleaningOperationsRecord) => {
      ...
      <DialogContent dir="rtl" className="max-h-[94vh] max-w-5xl overflow-y-auto border-cyan-100 p-0">
        <DialogHeader className="border-b border-slate-100 bg-slate-50/80 p-6 text-right">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-xl text-slate-900">{record ? kind === "container" ? "تعديل بيانات أصل الباقة التنظيف" : "تعديل بيانات السجل" : kind === "contract" ? "تسجيل تعاقد" : kind === "contract_line" ? "تسجيل إيجار باقة التنظيف" : `إضافة ${KIND_LABELS[kind]}`}</DialogTitle>
              <DialogDescription className="mt-1">{kind === "container" ? "حدّد بيانات الباقة التنظيف وحالتها التشغيلية الحالية ليعتمد عليها نظام التوفر والتأجير." : "بيانات تشغيلية محفوظة مباشرة في نظام الباقات التنظيف."}</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        <form onSubmit={event => {
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
            <div className="mb-4 flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                 <p className="text-sm font-black text-slate-900">{kind === "container" ? "بيانات الباقة التنظيف الأساسية" : "البيانات الأساسية"}</p>
                 <p className="mt-1 text-[11px] text-slate-400">{kind === "container" ? "اختر الحالة التي تعكس وضع الباقة التنظيف الفعلي الآن، ثم راجع البيانات قبل الحفظ." : "أدخل البيانات في الحقول المتساوية ثم راجعها قبل الحفظ."}</p>
              </div>
          ...
          <DialogFooter className="gap-2 border-t border-slate-100 pt-4">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="gap-2" data-testid="button-cancel-record"><X size={15} /> إلغاء</Button>
             <Button type="submit" disabled={busy} className="gap-2 bg-cyan-800 hover:bg-cyan-900" data-testid="button-save-record"><Save size={15} /> {busy ? "جارٍ الحفظ..." : kind === "container" ? "حفظ بيانات الباقة التنظيف" : "حفظ السجل"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
