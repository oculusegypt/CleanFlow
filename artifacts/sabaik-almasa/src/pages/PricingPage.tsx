...
const FACTORS = [
  { title: "مقاس الباقة التنظيف المطلوب", desc: "تتراوح المقاسات من 12 إلى 30 ياردة لمخلفات الأنقاض، و6 إلى 10 ياردة للنفايات التجارية." },
  { title: "نوع ووزن المخلفات", desc: "مخلفات الهدم الثقيلة والخرسانة المسلحة تختلف عن مخلفات الترميم الخفيفة أو النفايات العضوية." },
  { title: "مدة بقاء الباقة التنظيف والتفريغات", desc: "المدة القياسية 10 أيام للرد، مع إمكانية جدولة ردود متعددة أو عقود سنوية للمنشآت." },
...
const FAQS = [
  {
    q: "كم سعر إيجار باقة التنظيف الأنقاض 20 ياردة في الرياض؟",
    a: "سعر إيجار باقة التنظيف 20 ياردة هو 500 ريال للرد الواحد شامل التوصيل والسحب والتفريغ في المردم الرسمي لمدة بقاء تصل إلى 10 أيام.",
  },
  {
    q: "كم سعر إيجار باقة التنظيف الأنقاض الصغيرة 12 ياردة؟",
    a: "سعر باقة التنظيف 12 ياردة هو 400 ريال للرد، وهي مثالية لمشاريع الترميم الصغيرة والتعديلات الداخلية.",
  },
  {
  ...
  {
    q: "ما هي المدة المسموح بها لبقاء الباقة التنظيف في الموقع؟",
    a: "مدة بقاء الباقة التنظيف هي 10 أيام للرد الواحد، ويمكن تمديد المدة أو طلب السحب والتبديل في أي وقت بتواصل سريع.",
  },
  {
  ...
  {
    q: "كم يستغرق توصيل الباقة التنظيف بعد تأكيد الطلب؟",
    a: "يتم توصيل الباقة التنظيف لموقعك خلال ساعتين من تأكيد الطلب للطلبات الفورية، أو في الموعد المحدد للطلبات المجدولة.",
  },
]
...
export default function PricingPage() {
  ...
  const pricingWhatsappHref = `https://wa.me/966${(phoneWhatsapp || "0554498403").replace(/^0/, "")}?text=${encodeURIComponent("أريد الاستفسار عن أسعار تأجير الباقات التنظيف")}`
  const packagesWhatsappHref = `https://wa.me/966${(phoneWhatsapp || "0554498403").replace(/^0/, "")}?text=${encodeURIComponent("أريد طلب تأجير باقة التنظيف")}`

  useEffect(() => {
    if (!isLoaded) return
    const SITE_URL = getSiteUrl()
    document.title = companyName ? `أسعار تأجير الباقات التنظيف بالرياض | ${companyName}` : "أسعار تأجير الباقات التنظيف بالرياض | عروض حصرية وتوصيل فوري"
    ...
    setMeta("name", "description", `أسعار تأجير باقات التنظيف الأنقاض والنفايات والمكابس بالرياض ${companyName ? `من ${companyName}` : ""}. أسعار تبدأ من 400 ريال للرد شامل التوصيل والسحب والتفريغ في المرادم الرسمية.`)
    setMeta("name", "keywords", "أسعار تأجير الباقات التنظيف بالرياض, باقة التنظيف 12 ياردة, باقة التنظيف 20 ياردة, باقة التنظيف 30 ياردة, عقد نظافة بلدي, نقل مخلفات الهدم بالرياض")
    setMeta("property", "og:title", companyName ? `أسعار تأجير الباقات التنظيف بالرياض | ${companyName}` : "أسعار تأجير الباقات التنظيف بالرياض")
    ...
    setMeta("property", "og:image:alt", `أسعار تأجير الباقات التنظيف بالرياض — ${resolvedCompany}`)
    setMeta("property", "og:locale", "ar_SA")
    setMeta("property", "og:site_name", `${resolvedCompany} — تأجير باقات التنظيف بالرياض`)
    ...
    const schemas = [
      {
        ...
        "@type": "Service",
        "name": "تأجير باقات التنظيف الأنقاض والنفايات بالرياض",
        "url": `${SITE_URL}/pricing`,
        "description": "تأجير باقات التنظيف 12 و 15 و 20 و 30 ياردة لنقل مخلفات البناء والهدم وعقود النظافة الإلكترونية بالرياض",
        "provider": {
          "@type": "LocalBusiness",
        ...
        "hasOfferCatalog": {
          "@type": "OfferCatalog",
          "name": "قائمة أسعار الباقات التنظيف",
      ...
      },
      {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 2, "name": "أسعار تأجير الباقات التنظيف", "item": `${SITE_URL}/pricing` },
        ]
      }
    ]
    ...
    <div className="min-h-screen flex flex-col bg-gray-50" dir="rtl">
      ...
      <div className="bg-gradient-to-bl from-primary to-primary/80 text-white pt-24 pb-12 px-4">
        <div className="container mx-auto max-w-5xl">
          <nav className="flex items-center gap-2 text-white/60 text-sm mb-4">
            ...
            <ChevronLeft size={13} />
            <span className="text-white">أسعار تأجير الباقات التنظيف</span>
          </nav>
          <h1 className="text-3xl md:text-5xl font-black mb-3 leading-tight">
            أسعار تأجير الباقات التنظيف بالرياض
            <span className="block text-2xl md:text-3xl text-secondary mt-1">أسعار واضحة وشاملة التوصيل والسحب</span>
          </h1>
          <p className="text-white/80 text-lg max-w-2xl leading-relaxed">
            نوفر لك باقات التنظيف الأنقاض والنفايات بمختلف المقاسات في كافة أحياء الرياض. السعر يشمل التوصيل، مدة بقاء تصل إلى 10 أيام، والنقل والتفريغ في المرادم الرسمية.
          </p>
      ...
      <main className="flex-1 container mx-auto max-w-5xl px-4 py-12">
        ...
        <section className="mb-14">
          <h2 className="text-2xl font-black text-gray-900 mb-2">جدول مقاسات وأسعار الباقات التنظيف</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {PUBLIC_PRICING_PACKAGES.map(c => (
              <div key={c.name} className={`bg-white rounded-2xl border-2 overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col justify-between ${c.highlight ? "border-secondary shadow-secondary/10" : "border-gray-100"}`}>
                <div>
                  {c.highlight && (
                    <div className="bg-secondary text-white text-center py-1.5 text-xs font-black tracking-wide">الباقة التنظيف الأكثر طلباً لمشاريع البناء</div>
                  )}
                ...
                <div className="p-5 pt-0">
                  <button
                    onClick={() => openModal({ serviceType: "باقات التنظيف الأنقاض", containerName: c.name })}
        ...
        <section className="mb-14 bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
          <div className="p-6 border-b border-gray-100">
            <h2 className="text-xl font-black text-gray-900">مقارنة أسعار باقات التنظيف الأنقاض والنفايات بالرياض</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {[
                  ["باقة التنظيف 12 ياردة (10 م³)", "الترميمات والتعديلات الداخلية", "حتى 10 أيام", "400 ر.س"],
                  ["باقة التنظيف 15 ياردة (12 م³)", "أعمال التوسعة والتشطيب", "حتى 10 أيام", "450 ر.س"],
                  ["باقة التنظيف 20 ياردة (16 م³)", "بناء وهدم الفلل والعمائر", "حتى 10 أيام", "500 ر.س"],
                  ["باقة التنظيف 30 ياردة (22 م³)", "المشاريع الكبرى والهدم الشامل", "حتى 10 أيام", "700 ر.س"],
                  ["باقة التنظيف نفايات 6 - 10 ياردة", "المطاعم والمنشآت التجارية", "عقد سنوي", "حسب الموقع والنشاط"],
        ...
        {/* Factors */}
        <section className="mb-14">
          <h2 className="text-2xl font-black text-gray-900 mb-2">العوامل المحددة لتكلفة الباقة التنظيف</h2>
        ...
        {/* FAQs */}
        <section className="mb-14">
          <h2 className="text-2xl font-black text-gray-900 mb-6">الأسئلة الشائعة حول أسعار الباقات التنظيف</h2>
        ...
        <section className="bg-gradient-to-l from-primary to-primary/90 rounded-2xl p-8 text-white text-center">
          ...
          <h2 className="text-2xl font-black mb-2">هل تحتاج إلى توصيل باقة التنظيف فورياً لموقعك؟</h2>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            >
              <Box size={16} /> اطلب الباقة التنظيف الآن
            </button>
