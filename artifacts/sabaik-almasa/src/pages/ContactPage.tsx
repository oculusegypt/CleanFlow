...
export default function ContactPage() {
  ...
  const secondaryWhatsapp = phones.find(phone => phone !== phoneWhatsapp && phone !== phoneCall)
    ...
    || phoneCall
  const waHref = `https://wa.me/966${(phoneWhatsapp || "0554498403").replace(/^0/, "")}?text=${encodeURIComponent(companyName ? `مرحباً، أرغب في حجز باقة التنظيف من ${companyName}` : "مرحباً، أرغب في حجز باقة التنظيف")}`
    ...
    <div className="min-h-screen flex flex-col bg-gray-50" dir="rtl">
      ...
      <div className="bg-primary text-white py-12 px-4">
        <div className="container mx-auto">
          ...
          <p className="text-white/70 mt-2 text-lg">نحن هنا لخدمتك وتوصيل الباقة التنظيف لموقعك فوراً في أي وقت</p>
        </div>
      </div>
      ...
      <main className="flex-1 container mx-auto px-4 md:px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <a href={wa2Href} target="_blank" rel="noreferrer"
            className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col items-center text-center gap-3 hover:shadow-md hover:-translate-y-1 transition-all">
