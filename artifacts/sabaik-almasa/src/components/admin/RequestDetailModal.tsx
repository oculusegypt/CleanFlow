...
function buildRequestText(req: ServiceRequest, shortDirectionsUrl?: string): string {
  ...
  const lines = [
    ...
    `📦 حجم ومقاس الباقة التنظيف: ${req.packageSize || "غير محدد"}`,
    `📍 الموقع: ${req.location}`,
...
export default function RequestDetailModal({
  ...
  const readinessItems = [
    ...
    { label: "الخدمة والباقة التنظيف", complete: Boolean(request.serviceType.trim() && request.packageSize.trim()) },
          ...
          <div className="p-5 space-y-4">
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <InfoCell icon={<Package className="w-4 h-4 text-amber-400" />} label="حجم ومقاس الباقة التنظيف">
                  <span className="font-semibold">{request.packageSize || "غير محدد"}</span>
                </InfoCell>
                {request.duration && (
