...
export type RequestDocumentContext = Pick<ServiceRequest, "id" | "clientName" | "phone" | "email" | "serviceType" | "packageSize" | "location" | "duration" | "notes" | "appointmentType" | "scheduledAt">
...
export default function RequestDocumentModal({ request, kind, onClose }: Props) {
  ...
  const { data: snapshot } = useGetCleaningOperations({ query: { enabled: Boolean(request && (kind === "contract" || kind === "invoice")), queryKey: getGetCleaningOperationsQueryKey() } })
  const contractMutation = useCreateCleaningContractWorkflow()
  const invoiceMutation = useCreateCleaningOperationsRecord()
  ...
  const createInvoice = () => {
    ...
    invoiceMutation.mutate({
      ...
      onSuccess: created => {
        ...
        if (created?.id) navigate(`/admin/cleaning-operations/invoice/${created.id}/details`)
      },
    ...
    contractMutation.mutate({
      data: {
        ...
        assignment: {
          ...
          packageRecordId: payload.packageRecordId,
          contractNumber,
      ...
      onSuccess: result => {
        ...
        if (id) navigate(`/admin/cleaning-operations/contract/${id}/print`)
      },
