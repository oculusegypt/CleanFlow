import { getCleaningServiceStatus } from "@/lib/containerStatusVisuals"

export function CleaningServiceStatus({
  status,
  code,
  className = "",
  numberClassName = "",
  ...
  return (
    <span className={`relative inline-flex overflow-hidden ${className}`} style={{ containerType: "inline-size" }}>
      <img
        src={getCleaningServiceStatus(status)}
        alt={`باقة التنظيف ${code}`}
