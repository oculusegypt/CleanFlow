...
// ─── helpers ──────────────────────────────────────────────────────────────────
/** Build the packageSize string from a container record (matches homepage style) */
function packageSizeString(name: string, size: string) {
  return size ? `${name} - ${size}` : name
}

const ADMIN_SERVICE_OPTIONS = [
  { title: "باقات التنظيف الأنقاض", icon: "Box", category: "debris" as const },
  { title: "باقات التنظيف النفايات", icon: "Package", category: "waste" as const },
...
export default function RequestFormModal({ open, onClose, request, onSuccess }: Props) {
  ...
  const { data: containers = [] } = useGetPackages()
  const { data: customerRecords = [], isLoading: customersLoading } =
    useGetCleaningOperationsRecords({ kind: "customer" })
  const createCustomer = useCreateCleaningOperationsRecord()

  const { register, handleSubmit, setValue, watch, reset, formState: { isSubmitting } } = useForm<FormValues>({
    defaultValues: {
      clientName: "", phone: "", email: "", serviceType: "", packageSize: "",
  ...
  const showCleaningPackagePicker = !!selectedService?.category
  ...
  const activeCleaningPackages = containers.filter(c => c.isActive).sort((a, b) => a.order - b.order)
  const serviceCleaningPackages = selectedService?.category
    ? activeCleaningPackages.filter((container) => container.category === selectedService.category)
    : []
  ...
  const onSubmit = (values: FormValues) => {
    const shared = {
      ...
      packageSize:   selectedService?.category ? values.packageSize || null : "غير محدد",
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 mt-1">
                  ...
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      // clear container when service changes
                       if (!s.category) setValue("packageSize", "")
                    }}
          ...
          <AnimatePresence>
            {showCleaningPackagePicker && (
              ...
              >
                <Label className="text-sm font-semibold text-gray-800 mb-2 block">
                  حجم ومقاس الباقة التنظيف <span className="text-red-500">*</span>
                </Label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                   {serviceCleaningPackages.map(c => {
                    const val = packageSizeString(c.name, c.size)
                    const selected = packageSize === val
                    return (
                      <motion.button
                        key={c.id}
                        type="button"
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.97 }}
                        onClick={() => setValue("packageSize", val, { shouldValidate: true })}
                      ...
                      >
                        ...
                        <div className="h-16 overflow-hidden bg-gray-100 relative">
                          <img
                            src={resolvePackageImageUrl(c.imageUrl)}
                            alt={c.name}
                 ...
                 <input type="hidden" {...register("packageSize", { required: true })} />
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <DialogFooter className="flex gap-2 pt-1">
            ...
            <Button
              type="submit"
              disabled={isSubmitting || !serviceType || !!(showCleaningPackagePicker && !packageSize)}
            >
