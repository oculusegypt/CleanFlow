...
export function replaceLegacyCompanyName(value: string, companyName: string): string {
  ...
  let normalized = value
    .replace(/مؤسسة\s+سبائك\s+الماسة(\s+لتأجير\s+الباقات التنظيف(\s+ونقل\s+الأنقاض)?)?/g, resolvedName || "خدمات التنظيف بالرياض")
    .replace(/شركة\s+سبائك\s+الماسة/g, resolvedName || "خدمات التنظيف بالرياض")
    .replace(/باقات التنظيف\s+سبائك\s+الماسة(\s+المتاحة)?/g, resolvedName ? `باقات التنظيف ${resolvedName}` : "الباقات التنظيف المتاحة")
    .replace(/سبائك\s+الماسة/g, resolvedName || "خدمات التنظيف بالرياض")
    .replace(/مؤسسة\s+السهم\s+كلين(\s+لخدمات\s+التنظيف\s+بالرياض)?/g, resolvedName || "خدمات التنظيف بالرياض")
    .replace(/شركة\s+السهم\s+كلين/g, resolvedName || "خدمات التنظيف بالرياض")
    .replace(/السهم\s+كلين/g, resolvedName || "خدمات التنظيف بالرياض")
...
export function SiteSettingsProvider({ children }: { children: ReactNode }) {
  ...
  // Do not render pages against the empty/default settings object. That
  // creates a visible first frame with fallback copy, followed by a second
  // frame after the real site settings arrive.
  if (!settings.isLoaded) {
    return (
      <div className="min-h-screen bg-primary flex items-center justify-center" dir="rtl" aria-live="polite">
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 border-4 border-white/20 border-t-secondary rounded-full animate-spin" />
          <p className="text-white/70 text-sm">جاري تحميل إعدادات الموقع...</p>
        </div>
      </div>
    )
  }
