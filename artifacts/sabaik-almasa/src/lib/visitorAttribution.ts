...
const SESSION_ACTIVITY_KEY = "sab_sid_last_seen";
...
function getSessionId() {
  ...
  const lastSeen = Number(safeRead(SESSION_ACTIVITY_KEY) || 0);
  if (existing && lastSeen > 0 && Date.now() - lastSeen < SESSION_TIMEOUT_MS) {
    safeWrite(SESSION_ACTIVITY_KEY, String(Date.now()));
    return existing;
  }
...
export function getVisitorTracking(): VisitorTracking {
  ...
  const attributionKey = `${ATTRIBUTION_KEY}:${sessionId}`;
  try {
    const raw = safeRead(attributionKey);
  ...
  if (!stored?.landingPage) {
    ...
    safeWrite(attributionKey, JSON.stringify(stored));
  }
...
export function playNotificationChime(storageKey?: string) {
  try {
    ...
    if (typeof window !== "undefined") {
      ...
      if (
        !isAdmin &&
        (localStorage.getItem("chat_sound_muted") === "true" ||
          localStorage.getItem("sound_muted") === "true")
      )
        return;
    }
  ...
  try {
    ...
    let convId = Number(
      sessionStorage.getItem("support_conversation_id") ||
        localStorage.getItem("support_conversation_id") ||
        "0",
    );
    let orderId = Number(
    ...
    const sessionKeys = [
      "sabaik_live_chat_session",
      "hawiat_live_chat_session",
      "alsahm_live_chat_session",
      "cleanflow_live_chat_session",
    ];
      ...
      if (chatRaw) {
        try {
          ...
          if (parsed.conversationId && !convId)
            convId = Number(parsed.conversationId);
        } catch {}
      }
    }
