...
router.get("/ads", async (req, res) => {
  try {
    ...
    const filters = [eq(adsTable.isActive, true)]
    if (position) filters.push(eq(adsTable.position, position))
    if (type) filters.push(eq(adsTable.type, type))
    const rows = position
      ? await db.select().from(adsTable).where(and(...filters)).orderBy(asc(adsTable.order))
      : await db.select().from(adsTable).where(and(...filters)).orderBy(asc(adsTable.order));
