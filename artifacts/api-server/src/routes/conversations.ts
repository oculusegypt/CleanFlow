...
import { conversationsTable, messagesTable, packagesTable, activeVisitorsTable } from "@workspace/db";
...
router.post("/conversations", async (req, res) => {
  ...
  const sessionId = typeof req.body?.sessionId === "string"
    ? req.body.sessionId.trim().slice(0, 160)
    : "";
  ...
  // If this conversation was started from an operator invitation, associate
  // it with the same live visitor session and consume the invitation.
  if (sessionId) {
...
router.post(["/conversations/:id/messages", "/admin/conversations/:id/messages"], requireAdminSender, async (req, res) => {
  ...
  if (normalizedMessageType === "package_form") {
    ...
    const [container] = await db.select({ id: packagesTable.id })
      .from(packagesTable)
      .where(eq(packagesTable.id, containerId));
    if (!container) {
