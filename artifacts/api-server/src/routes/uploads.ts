...
function uploadedImageName(_value: string): string {
  // Keep uploaded URLs ASCII-only. Shared hosting and some CDN/proxy layers
  // can decode an Arabic filename differently from the filesystem filename,
  // which turns an otherwise valid upload into a 404 after deployment.
  return `upload-${Date.now()}-${crypto.randomBytes(6).toString("hex")}.webp`
}
...
function safeUpload(req: Request, res: Response, next: NextFunction): void {
  upload.single("file")(req, res, (error: unknown) => {
    ...
    if (req.file) {
      ...
      const semanticName = uploadedImageName(String(req.body?.title || req.body?.alt || ""));
      const semanticPath = path.join(UPLOADS_DIR, semanticName);
      fs.renameSync(req.file.path, semanticPath);
      req.file.path = semanticPath;
      req.file.filename = semanticName;
    }
    next();
  });
}
...
router.post("/admin/uploads", requireAdmin, requireSectionPermission("settings"), safeUpload, rejectUnsafeFileName, (req: Request, res: Response): void => {
  ...
  res.json({ url: encodeURI(url), filename: req.file.filename, alt: String(req.body?.title || "صورة خدمات التنظيف بالرياض"), size: req.file.size, contentType: req.file.mimetype });
});
...
router.post("/driver/uploads", requireAdmin, requireDriver, safeUpload, rejectUnsafeFileName, (req: Request, res: Response): void => {
  if (!req.file) { res.status(400).json({ error: "لم يُرفَق ملف" }); return; }
  const url = `/api/uploads/${req.file.filename}`;
  res.json({ url: encodeURI(url), filename: req.file.filename, alt: String(req.body?.title || "صورة مرفقة من العميل"), size: req.file.size, contentType: req.file.mimetype });
});
...
router.delete("/admin/uploads/:filename", requireAdmin, requireSectionPermission("settings"), (req: Request, res: Response): void => {
  ...
  if (filename !== req.params["filename"] || !/^[\p{L}\p{N}._-]+$/u.test(filename)) {
    res.status(400).json({ error: "اسم الملف غير صالح" });
    return;
  }
