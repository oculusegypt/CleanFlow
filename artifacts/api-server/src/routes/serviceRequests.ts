...
import { db, adminsTable, serviceRequestsTable, conversationsTable, messagesTable, activeVisitorsTable, cleaningOperationsAuditTable, cleaningOperationsRecordsTable } from "@workspace/db";
...
function parseCleaningPackagePayload(payload: string): Record<string, unknown> {
  try {
...
function isCleaningPackageWork(request: typeof serviceRequestsTable.$inferSelect) {
...
function operationalRecordAsWorkOrder(record: typeof cleaningOperationsRecordsTable.$inferSelect) {
  const payload = parseCleaningPackagePayload(record.payload);
  return {
    id: record.id,
    ...
    packageSize: String(payload.containerCode ?? ""),
    propertyType: null,
    areaSize: null,
    ...
    packageRecordId: Number(payload.packageRecordId ?? 0) || null,
...
async function prepareCleaningPackageCompletion(request: typeof serviceRequestsTable.$inferSelect) {
  if (!isCleaningPackageWork(request)) return null;
  const contract = await db.select().from(cleaningOperationsRecordsTable)
    .where(eq(cleaningOperationsRecordsTable.id, request.contractRecordId!)).get();
  if (!contract || contract.kind !== "contract" || contract.status === "archived") {
    throw new Error("العقد المرتبط بأمر العمل غير موجود");
  }
  const contractPayload = parseCleaningPackagePayload(contract.payload);
  const containerCode = String(contractPayload.containerCode ?? "").trim();
  if (!containerCode) throw new Error("العقد المرتبط لا يحتوي على رقم باقة التنظيف");
  const assets = await db.select().from(cleaningOperationsRecordsTable);
  const asset = assets.find(record =>
    ["container", "container_asset"].includes(record.kind) &&
    record.status !== "archived" &&
    containerCodeFrom(parseCleaningPackagePayload(record.payload)) === containerCode,
  );
  if (!asset) throw new Error("أصل الباقة التنظيف المرتبط بأمر العمل غير موجود");
  return {
    contract,
    contractPayload,
    asset,
...
function syncCleaningPackageCompletion(
  writer: DbWriter,
  request: typeof serviceRequestsTable.$inferSelect,
  prepared: Awaited<ReturnType<typeof prepareCleaningPackageCompletion>>,
  actorId: number,
) {
  if (!prepared) return;
  ...
  const nextAssetPayload = {
    ...parseCleaningPackagePayload(prepared.asset.payload),
  ...
  if (!prepared.emptying) {
    writer.update(cleaningOperationsRecordsTable).set({
      status: contractStatus,
      payload: JSON.stringify(nextContractPayload),
      updatedAt: now,
    }).where(eq(cleaningOperationsRecordsTable.id, prepared.contract.id)).run();
    writer.update(cleaningOperationsRecordsTable).set({
      status: assetStatus,
      payload: JSON.stringify(nextAssetPayload),
      updatedAt: now,
    }).where(eq(cleaningOperationsRecordsTable.id, prepared.asset.id)).run();
  } else {
    writer.update(cleaningOperationsRecordsTable).set({
  ...
  const movement = writer.insert(cleaningOperationsRecordsTable).values({
  ...
  writer.insert(cleaningOperationsAuditTable).values([
    {
...
router.post("/admin/service-requests/from-contract", requireAdmin, requireAnySectionPermission("requests", "container_system"), async (req, res) => {
  ...
  const containerId = Number(packageRecordId);
  if (!Number.isInteger(contractId) || contractId <= 0 || !Number.isInteger(customerId) || customerId <= 0 || !Number.isInteger(containerId) || containerId <= 0) {
    return res.status(422).json({ error: "ربط أمر العمل بالعميل والعقد وأصل الباقة التنظيف مطلوب" });
  }
  ...
  const requestedServiceType = String(serviceType ?? "تسليم باقة التنظيف").trim();
  ...
  const [contract] = await db.select().from(cleaningOperationsRecordsTable)
    .where(eq(cleaningOperationsRecordsTable.id, contractId));
  if (!contract || contract.kind !== "contract" || contract.status === "archived") {
    return res.status(422).json({ error: "العقد المرتبط بأمر العمل غير موجود" });
  }
  const contractPayload = parseCleaningPackagePayload(contract.payload);
  const [customerRecord] = await db.select().from(cleaningOperationsRecordsTable)
    .where(eq(cleaningOperationsRecordsTable.id, customerId));
  const [containerRecord] = await db.select().from(cleaningOperationsRecordsTable)
    .where(eq(cleaningOperationsRecordsTable.id, containerId));
  const customerPayload = parseCleaningPackagePayload(customerRecord?.payload);
  const containerPayload = parseCleaningPackagePayload(containerRecord?.payload);
  ...
  const containerMatches = Number(contractPayload.packageRecordId) === containerId ||
    (!contractPayload.packageRecordId && containerCode !== "" &&
  ...
  if (!customerRecord || customerRecord.kind !== "customer" || !containerRecord ||
    ...
    return res.status(409).json({ error: "علاقات أمر العمل لا تطابق العميل أو أصل الباقة التنظيف في العقد" });
  }
  ...
  const existingWorkOrder = (await db.select().from(cleaningOperationsRecordsTable))
    .find(row => row.kind === "work_order" && row.status !== "archived" &&
      parseCleaningPackagePayload(row.payload).operationKey === operationKey);
  ...
  const operation = String(requestedServiceType).match(/تفريغ|empty/i) ? "COMPLETE_CLEANING_SERVICE" :
    String(requestedServiceType).match(/استرجاع|سحب|pickup|return/i) ? "ASSIGN_CLEANING_SERVICE" : "SCHEDULE_CLEANING_SERVICE";
  const [appointment] = await db.insert(cleaningOperationsRecordsTable).values({
    kind: "appointment",
    status: "scheduled",
  ...
  const workOrderPayload = {
    ...
    packageSize: String(packageSize ?? ""), location: String(location ?? "يحدد لاحقًا"),
  ...
  const [workOrder] = await db.insert(cleaningOperationsRecordsTable).values({
  ...
  await db.update(cleaningOperationsRecordsTable).set({
    payload: JSON.stringify({
      ...parseCleaningPackagePayload(appointment.payload),
      workOrderRecordId: workOrder.id,
    }),
    updatedAt: new Date().toISOString(),
  }).where(eq(cleaningOperationsRecordsTable.id, appointment.id));
...
router.post("/service-requests", async (req, res) => {
  ...
  if (conversationId !== null) {
    const detailsContent = [
      ...
      request.packageSize ? `الباقة / المقاس: ${request.packageSize}` : "",
...
router.get("/service-requests/:id", async (req, res) => {
  ...
  const {
    id: requestId, clientName, phone, serviceType, packageSize, status,
...
router.patch("/service-requests/:id", requireAdmin, requireSectionPermission("requests"), async (req, res) => {
  ...
  const id = parseInt(String(req.params.id), 10);
  const {
    clientName, phone, email, serviceType, packageSize, location,
    duration, notes, appointmentType, scheduledAt, status, adminNotes,
    customerRecordId, packageRecordId, contractRecordId,
  } = req.body;
  ...
  if (packageRecordId !== undefined) updateData.packageRecordId = packageRecordId === null ? null : Number(packageRecordId);
...
router.patch("/service-requests/:id/assignment", requireAdmin, requireSectionPermission("work_orders"), requireRequestAssignment, async (req, res) => {
  ...
  if (vehicleId !== null) {
    const vehicle = await db.select().from(cleaningOperationsRecordsTable)
      .where(eq(cleaningOperationsRecordsTable.id, vehicleId)).get();
    ...
    const vehiclePayload = parseCleaningPackagePayload(vehicle.payload);
    vehiclePlate = String(vehiclePayload.vehiclePlate ?? vehiclePayload.plateNumber ?? vehiclePayload.plate ?? vehicle.reference ?? "").trim();
  }

  const operationalRow = await db.select().from(cleaningOperationsRecordsTable)
    .where(eq(cleaningOperationsRecordsTable.id, id)).get();
  if (operationalRow?.kind === "work_order") {
    const current = operationalRecordAsWorkOrder(operationalRow);
    const currentPayload = parseCleaningPackagePayload(operationalRow.payload);
...
router.get("/driver/work-orders", requireAdmin, requireDriver, async (req, res) => {
  ...
  const operationalRows = await db.select().from(cleaningOperationsRecordsTable);
...
router.patch("/driver/work-orders/:id", requireAdmin, requireDriver, async (req, res) => {
  ...
  if (operationalRow?.kind === "work_order") {
    ...
    let preparedCleaningPackageCompletion: Awaited<ReturnType<typeof prepareCleaningPackageCompletion>> = null;
    if (nextStatus === "completed") {
      try {
        preparedCleaningPackageCompletion = await prepareCleaningPackageCompletion(current);
      } catch (error) {
        return res.status(422).json({ error: error instanceof Error ? error.message : "تعذر التحقق من ارتباط الباقة التنظيف بالعقد" });
      }
      if (preparedCleaningPackageCompletion && (!receiverName || !signatureData || !proofPhotoUrl)) {
        return res.status(422).json({ error: "يلزم تسجيل اسم المستلم وتوقيع العميل وصورة إثبات قبل إكمال حركة الباقة التنظيف" });
      }
    }
    const now = new Date().toISOString();
    const currentPayload = parseCleaningPackagePayload(operationalRow.payload);
    ...
    const updated = db.transaction((tx) => {
      ...
      if (nextStatus === "completed" && preparedCleaningPackageCompletion) {
        syncCleaningPackageCompletion(tx, operationalRecordAsWorkOrder(next), preparedCleaningPackageCompletion, adminRequest.adminId);
      }
      tx.insert(cleaningOperationsAuditTable).values({
  ...
  if (nextStatus === "completed") {
    try {
      preparedCleaningPackageCompletion = await prepareCleaningPackageCompletion(request);
    } catch (error) {
      return res.status(422).json({
        error: error instanceof Error ? error.message : "لا يمكن إكمال أمر العمل قبل اكتمال ربط العقد والباقة التنظيف",
      });
    }
    if (preparedCleaningPackageCompletion && (!receiverName || !signatureData || !proofPhotoUrl)) {
      return res.status(422).json({ error: "يلزم تسجيل اسم المستلم وتوقيع العميل وصورة إثبات قبل إكمال حركة الباقة التنظيف" });
  ...
  try {
    updated = db.transaction((tx) => {
      if (operationKey) {
        const previous = tx.select().from(cleaningOperationsAuditTable)
          .where(and(
            eq(cleaningOperationsAuditTable.recordId, id),
            eq(cleaningOperationsAuditTable.kind, "service_request"),
            eq(cleaningOperationsAuditTable.action, "driver_status_transition"),
          )).all().find((audit) => {
            const payload = parseCleaningPackagePayload(audit.afterPayload ?? "");
      ...
      if (nextStatus === "completed" && preparedCleaningPackageCompletion) {
        syncCleaningPackageCompletion(tx, nextRequest, preparedCleaningPackageCompletion, adminRequest.adminId);
      }
