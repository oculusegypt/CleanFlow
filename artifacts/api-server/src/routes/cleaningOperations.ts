...
import { db, sqlite, cleaningOperationsAuditTable, cleaningOperationsRecordsTable, serviceRequestsTable, financialTruth, financialPeriods, closeFinancialPeriod, postToFinancialCore, reverseInFinancialCore } from "@workspace/db";
...
function postedCollections(rows: typeof cleaningOperationsRecordsTable.$inferSelect[]) {
...
async function findByOperationKey(kind: string, operationKey: string) {
  ...
  const rows = await db.select().from(cleaningOperationsRecordsTable);
...
function formatRecord(row: typeof cleaningOperationsRecordsTable.$inferSelect) {
...
function requireCleaningPackagePermission(kind: string) {
...
const MOVEMENT_STATUS_BY_TYPE: Record<string, string> = {
  ...
  swap: "in_transit",
  "تبديل": "in_transit",
  "تبديل باقة التنظيف": "in_transit",
  transport: "in_transit",
  move: "in_transit",
  "نقل": "in_transit",
  "نقل باقة التنظيف": "in_transit",
  "توصيل": "in_transit",
  "توصيل باقة التنظيف": "in_transit",
  "رفع": "in_transit",
  "رفع باقة التنظيف": "in_transit",
  "تحميل": "in_transit",
...
function movementTransitionAllowed(currentStatus: string, movementType: string) {
  ...
  if (["replacement", "swap", "تبديل", "تبديل باقة التنظيف"].includes(type)) return ["rented", "with_customer", "awaiting_return", "in_transit"].includes(current);
  if (["transport", "move", "نقل", "نقل باقة التنظيف", "توصيل", "توصيل باقة التنظيف", "رفع", "رفع باقة التنظيف", "تحميل"].includes(type)) {
...
async function findAssetByCode(containerCode: string) {
  ...
  const records = await db.select().from(cleaningOperationsRecordsTable);
...
async function validateContractPayload(payload: Record<string, unknown>, ignoredId?: number, allowGeneratedSite = false) {
  ...
  if (["active", "issued", "scheduled", "delivered"].includes(lifecycleStatus) && !containerCode) {
    throw new Error("العقد التشغيلي يجب أن يرتبط بباقة التنظيف");
  }
  if (containerCode && !(await findAssetByCode(containerCode))) {
    throw new Error("الباقة التنظيف المرتبطة بالعقد غير موجودة");
  }
  ...
  if (await hasOverlappingContract(payload, ignoredId)) {
    throw new Error("الباقة التنظيف مرتبطة بعقد آخر خلال نفس الفترة");
  }
}
...
async function validateFinancialPayload(kind: string, payload: Record<string, unknown>) {
  ...
  if (kind === "transfer") {
    ...
    const treasuries = await db.select().from(cleaningOperationsRecordsTable);
  ...
  if (Number.isInteger(contractRecordId) && contractRecordId > 0 && ["payment", "receipt", "invoice", "invoice_return"].includes(kind)) {
    const contract = await db.select().from(cleaningOperationsRecordsTable)
      .where(eq(cleaningOperationsRecordsTable.id, contractRecordId)).get();
    ...
    if (kind === "invoice") {
      payload.description = payload.description || contractPayload.description || contractPayload.rentType || contractPayload.containerType || "خدمات باقات التنظيف";
  ...
  const customerRecordId = Number(payload.customerRecordId);
  if (customerRecordId) {
    const customer = await db.select().from(cleaningOperationsRecordsTable)
      .where(eq(cleaningOperationsRecordsTable.id, customerRecordId)).get();
  ...
  if (["salary_advance", "salary_payment", "commission"].includes(kind)) {
    ...
    const employee = await db.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, employeeId)).get();
  ...
  if (["stock_issue", "stock_issue_return", "purchase"].includes(kind)) {
    ...
    const warehouse = await db.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, warehouseId)).get();
  ...
  const contextKinds: Array<[string, string, string]> = [
    ...
    ["packageRecordId", "containerId", "container_asset"],
...
async function hasOverlappingContract(payload: Record<string, unknown>, ignoredId?: number) {
  ...
  const existing = await db.select().from(cleaningOperationsRecordsTable);
...
async function syncMovement(payload: Record<string, unknown>, actorId: number | null) {
  ...
  if (!nextStatus) throw new Error("نوع حركة الباقة التنظيف غير مدعوم");
  const asset = await findAssetByCode(String(payload.containerCode ?? ""));
  if (!asset) throw new Error("الباقة التنظيف المرتبطة بالحركة غير موجودة");
  if (!movementTransitionAllowed(asset.status, movementType)) {
    throw new Error(`لا يمكن تنفيذ حركة ${String(payload.movementType)} على باقة التنظيف حالتها الحالية ${asset.status}`);
  }
  ...
  await db.update(cleaningOperationsRecordsTable).set({
    status: nextStatus, payload: JSON.stringify(nextPayload), updatedAt: new Date().toISOString(),
  }).where(eq(cleaningOperationsRecordsTable.id, asset.id));
  await db.insert(cleaningOperationsAuditTable).values({
...
async function linkContractToRequest(payload: Record<string, unknown>, contractId: number, actorId: number | null) {
  ...
  const allRecords = await db.select().from(cleaningOperationsRecordsTable);
  ...
  if (!customer && customerName) {
    const [createdCustomer] = await db.insert(cleaningOperationsRecordsTable).values({
      kind: "customer",
      status: "active",
  ...
  await db.update(serviceRequestsTable).set({
    ...
    packageRecordId: Number(payload.packageRecordId ?? container?.id ?? 0) || null,
    contractRecordId: contractId,
    status: ["draft", "cancelled"].includes(String(payload.status ?? "")) ? request.status : "in_progress",
    adminNotes: `${request.adminNotes ?? ""}\nمرتبط بعقد الباقات التنظيف ${contractId}`.trim(),
    updatedAt: new Date().toISOString(),
  }).where(eq(serviceRequestsTable.id, requestId));
  await db.insert(cleaningOperationsAuditTable).values({
    recordId: contractId, kind: "contract", action: "request_link",
    afterPayload: JSON.stringify({ requestId, customerRecordId: payload.customerRecordId ?? null, packageRecordId: payload.packageRecordId ?? null }),
    actorId,
  });
}

router.get("/admin/cleaning-operations", requireCleaningPackagePermission("container_system"), async (req, res) => {
  const rows = await db.select().from(cleaningOperationsRecordsTable).orderBy(desc(cleaningOperationsRecordsTable.updatedAt));
  ...
  return res.json({
    ...
    summary: {
      ...
      availableCleaningPackages: records.filter(r => ["container", "container_asset"].includes(r.kind) && r.status === "available").length,
      rentedCleaningPackages: records.filter(r => ["container", "container_asset"].includes(r.kind) && r.status === "rented").length,
...
router.get("/admin/cleaning-operations/records", requireCleaningPackagePermission("container_system"), async (req, res) => {
  const kind = typeof req.query.kind === "string" ? req.query.kind : undefined;
  const status = typeof req.query.status === "string" ? req.query.status : undefined;
  const search = typeof req.query.search === "string" ? req.query.search.trim() : "";
  const filters = [];
  if (kind) filters.push(eq(cleaningOperationsRecordsTable.kind, kind));
  if (status) filters.push(eq(cleaningOperationsRecordsTable.status, status));
  if (search) filters.push(like(cleaningOperationsRecordsTable.payload, `%${search}%`));
  const rows = await db.select().from(cleaningOperationsRecordsTable)
    .where(filters.length ? and(...filters) : undefined)
    .orderBy(desc(cleaningOperationsRecordsTable.updatedAt));
  return res.json(rows.map(formatRecord));
});

router.post("/admin/cleaning-operations/contracts/workflow", requireCleaningPackagePermission("contract"), async (req, res) => {
  ...
  const assetId = Number(assignmentPayload.packageRecordId ?? contractPayload.packageRecordId);
  if (![customerId, assetId].every(value => Number.isInteger(value) && value > 0)) {
    return res.status(422).json({ error: "العميل وأصل الباقة التنظيف مطلوبان لإنشاء دورة العقد" });
  }
  try {
    await validateContractPayload(contractPayload, undefined, true);
    if (await hasOverlappingContract(contractPayload)) return res.status(409).json({ error: "الباقة التنظيف مرتبطة بعقد آخر خلال نفس الفترة" });
  } catch (error) {
    return res.status(422).json({ error: error instanceof Error ? error.message : "بيانات العقد غير صحيحة" });
  }
  try {
    const result = db.transaction((tx) => {
      const customer = tx.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, customerId)).get();
      let site = siteId > 0
        ? tx.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, siteId)).get()
        : undefined;
      const asset = tx.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, assetId)).get();
      ...
      if (!site) {
        ...
        site = tx.insert(cleaningOperationsRecordsTable).values({
          kind: "customer_site",
          status: "active",
      ...
      if (!asset || !["container", "container_asset"].includes(asset.kind) || asset.status === "archived") throw new Error("أصل الباقة التنظيف غير موجود");
      ...
      if (!["available", "reserved", "active"].includes(assetAvailability)) throw new Error("الباقة التنظيف ليست متاحة للتخصيص");
      contractPayload.siteRecordId = siteId;
      assignmentPayload.siteRecordId = siteId;
      const activeAssignment = tx.select().from(cleaningOperationsRecordsTable).all().find(record =>
        ...
        Number(parsePayload(record.payload).packageRecordId) === assetId,
      );
      if (activeAssignment) throw new Error("الباقة التنظيف مرتبطة بتخصيص نشط بالفعل");
      const now = new Date().toISOString();
      const contract = tx.insert(cleaningOperationsRecordsTable).values({
      ...
      const finalizedContract = tx.update(cleaningOperationsRecordsTable).set({
        reference: contractNumber,
        payload: JSON.stringify({ ...contractPayload, contractNumber }),
        updatedAt: now,
      }).where(eq(cleaningOperationsRecordsTable.id, contract.id)).returning().get();
      const assignment = tx.insert(cleaningOperationsRecordsTable).values({
        kind: "container_assignment", status: "reserved",
        reference: referenceFor("container_assignment", assignmentPayload, Date.now()),
        payload: JSON.stringify({ ...assignmentPayload, contractRecordId: contract.id, siteRecordId: siteId, packageRecordId: assetId, contractNumber, assignmentStatus: "reserved" }),
        createdBy: adminReq.adminId,
      }).returning().get();
      ...
      tx.update(cleaningOperationsRecordsTable).set({ status: "reserved", payload: JSON.stringify(nextAssetPayload), updatedAt: now }).where(eq(cleaningOperationsRecordsTable.id, assetId)).run();
      const appointment = tx.insert(cleaningOperationsRecordsTable).values({
        kind: "appointment", status: "scheduled",
        reference: referenceFor("appointment", appointmentPayload, Date.now()),
        payload: JSON.stringify({ ...appointmentPayload, contractRecordId: contract.id, contractNumber, customerRecordId: customerId, packageRecordId: assetId }),
        createdBy: adminReq.adminId,
      }).returning().get();
      const customerPayload = parsePayload(customer.payload);
      let serviceRequest;
      ...
      const workOrderPayload = {
        ...
        operationType: String(appointmentPayload.appointmentType ?? "delivery") === "pickup" ? "ASSIGN_CLEANING_SERVICE" : "SCHEDULE_CLEANING_SERVICE",
        serviceType: String(servicePayload.serviceType ?? "تسليم باقة التنظيف"),
      ...
      const workOrder = tx.insert(cleaningOperationsRecordsTable).values({
        kind: "work_order",
        status: "new",
      ...
      tx.update(cleaningOperationsRecordsTable).set({
        payload: JSON.stringify({ ...parsePayload(appointment.payload), workOrderRecordId: workOrder.id }),
        updatedAt: now,
      }).where(eq(cleaningOperationsRecordsTable.id, appointment.id)).run();
      ...
      let invoice: typeof cleaningOperationsRecordsTable.$inferSelect | null = null;
      if (billable) {
        ...
        const existingInvoice = tx.select().from(cleaningOperationsRecordsTable).all().find(row => {
          if (row.kind !== "invoice" || row.status === "archived") return false;
          const payload = parsePayload(row.payload);
        ...
        if (existingInvoice) {
          ...
          const lineItem = {
            description: containerType ? `باقة التنظيف ${containerCode} — ${containerType}` : `باقة التنظيف ${containerCode}`,
            containerCode,
            quantity: 1,
            unitPrice: subtotal,
            amount: subtotal,
          ...
          const insertedInvoice = tx.insert(cleaningOperationsRecordsTable).values({
            kind: "invoice",
            status: "draft",
          ...
          invoice = tx.update(cleaningOperationsRecordsTable).set({
            ...
            updatedAt: now,
          }).where(eq(cleaningOperationsRecordsTable.id, insertedInvoice.id)).returning().get();
          tx.insert(cleaningOperationsAuditTable).values({
            recordId: invoice.id,
            kind: "invoice",
      ...
      for (const record of [finalizedContract, assignment, appointment, workOrder]) {
        tx.insert(cleaningOperationsAuditTable).values({ recordId: record.id, kind: record.kind, action: "workflow_create", afterPayload: record.payload, actorId: adminReq.adminId }).run();
      }
...
router.get("/admin/cleaning-operations/financial/contract-ledgers", requireCleaningPackagePermission("container_system_ledger_entry"), async (req, res) => {
...
router.post("/admin/cleaning-operations/financial/settle", requireCleaningPackagePermission("payment"), async (req, res) => {
  const adminReq = req as unknown as AdminRequest;
  ...
  if (existing) {
    ...
    const ledger = (await db.select().from(cleaningOperationsRecordsTable)).find(row =>
  ...
  try {
    const result = db.transaction((tx) => {
      const all = tx.select().from(cleaningOperationsRecordsTable).all();
      ...
      let deposit: typeof cleaningOperationsRecordsTable.$inferSelect | undefined;
      if (body.depositId) {
      ...
      const payment = tx.insert(cleaningOperationsRecordsTable).values({
      ...
      const ledger = tx.insert(cleaningOperationsRecordsTable).values({
      ...
      for (const item of contractRows) {
        ...
        tx.update(cleaningOperationsRecordsTable).set({ payload: JSON.stringify(nextContract), status: nextStatus, updatedAt: now })
          .where(eq(cleaningOperationsRecordsTable.id, item.contract.id)).run();
        if (item.invoiceId) {
          const invoice = all.find(row => row.id === item.invoiceId && row.kind === "invoice" && row.status !== "archived");
          if (invoice) {
            const invoicePayload = parsePayload(invoice.payload);
            ...
            tx.update(cleaningOperationsRecordsTable).set({
              ...
              updatedAt: now,
            }).where(eq(cleaningOperationsRecordsTable.id, invoice.id)).run();
          }
        }
      }
      tx.insert(cleaningOperationsAuditTable).values([
      ...
      if (deposit) {
        ...
        tx.update(cleaningOperationsRecordsTable).set({
          ...
          updatedAt: now,
        }).where(eq(cleaningOperationsRecordsTable.id, deposit.id)).run();
      }
    ...
    if (operationKey && error instanceof Error && error.message.includes("UNIQUE constraint failed")) {
      ...
      if (raced) {
        ...
        const racedLedger = (await db.select().from(cleaningOperationsRecordsTable)).find(row =>
...
function matchContractForSettlement(row: typeof cleaningOperationsRecordsTable.$inferSelect, all: typeof cleaningOperationsRecordsTable.$inferSelect[]) {
  const payload = parsePayload(row.payload);
  const direct = String(payload.contractNumber ?? "").trim();
...
router.post("/admin/cleaning-operations/contracts/:id/lifecycle", requireCleaningPackagePermission("contract"), async (req, res) => {
  const adminReq = req as unknown as AdminRequest;
  ...
  try {
    const result = db.transaction((tx) => {
      const contract = tx.select().from(cleaningOperationsRecordsTable)
        .where(eq(cleaningOperationsRecordsTable.id, contractId)).get();
      if (!contract || contract.kind !== "contract" || contract.status === "archived") {
      ...
      if (action === "return" && contract.status !== "delivered") {
        throw new Error("لا يمكن استرجاع الباقة التنظيف قبل تسجيل تسليمها");
      }
      if (action === "approve" || action === "reject") {
        ...
        const updatedContract = tx.update(cleaningOperationsRecordsTable).set({
          status: nextStatus,
          payload: JSON.stringify(nextPayload),
          updatedAt: now,
        }).where(eq(cleaningOperationsRecordsTable.id, contractId)).returning().get();
        tx.insert(cleaningOperationsAuditTable).values({
          recordId: contractId,
          kind: "contract",
      ...
      if (!containerCode) throw new Error("العقد لا يحتوي على رقم باقة التنظيف");
      const asset = tx.select().from(cleaningOperationsRecordsTable).all().find(row =>
        ["container", "container_asset"].includes(row.kind) &&
        row.status !== "archived" &&
        assetCodeOf(parsePayload(row.payload)) === containerCode,
      );
      if (!asset) throw new Error("الباقة التنظيف المرتبطة بالعقد غير موجودة");
      const linkedRequest = tx.select().from(serviceRequestsTable)
        .where(eq(serviceRequestsTable.contractRecordId, contractId)).get();
      if (linkedRequest && linkedRequest.packageRecordId !== asset.id) {
        throw new Error("الطلب المرتبط لا يطابق أصل الباقة التنظيف في العقد");
      }
      if (!movementTransitionAllowed(asset.status, action)) {
        throw new Error(`لا يمكن تنفيذ ${action === "deliver" ? "التسليم" : "الاسترجاع"} على باقة التنظيف حالتها الحالية ${asset.status}`);
      }
      ...
      tx.update(cleaningOperationsRecordsTable).set({
        ...
        updatedAt: now,
      }).where(eq(cleaningOperationsRecordsTable.id, asset.id)).run();
      const movement = tx.insert(cleaningOperationsRecordsTable).values({
...
router.post("/admin/cleaning-operations/records", async (req, res) => {
  const adminReq = req as unknown as AdminRequest;
  ...
  if (kind === "contract") {
    ...
    const containerCode = String(payload.containerCode ?? "").trim();
    if (containerCode) {
      if (await hasOverlappingContract(payload)) return res.status(409).json({ error: "الباقة التنظيف مرتبطة بعقد آخر خلال نفس الفترة" });
    }
  ...
  if (kind === "container_assignment") {
    const contractRecordId = Number(payload.contractRecordId);
    const packageRecordId = Number(payload.packageRecordId);
    const siteRecordId = Number(payload.siteRecordId);
    if (![contractRecordId, packageRecordId, siteRecordId].every(value => Number.isInteger(value) && value > 0)) {
      return res.status(422).json({ error: "العقد وأصل الباقة التنظيف وموقع العميل مطلوبة للتخصيص" });
    }
    const records = await db.select().from(cleaningOperationsRecordsTable);
    const contract = records.find(record => record.id === contractRecordId && record.kind === "contract" && record.status !== "archived");
    const asset = records.find(record => record.id === packageRecordId && ["container", "container_asset"].includes(record.kind) && record.status !== "archived");
    const site = records.find(record => record.id === siteRecordId && record.kind === "customer_site" && record.status !== "archived");
    if (!contract || !asset || !site) return res.status(422).json({ error: "العقد أو الباقة التنظيف أو موقع العميل غير موجود" });
    const contractPayload = parsePayload(contract.payload);
    ...
    if (!["available", "reserved"].includes(canonicalAssetStatus(asset.status, asset.status))) {
      return res.status(409).json({ error: "الباقة التنظيف ليست متاحة للتخصيص" });
    }
    const activeAssignment = records.find(record =>
      record.kind === "container_assignment" &&
      record.status !== "archived" &&
      ["reserved", "active"].includes(String(parsePayload(record.payload).assignmentStatus ?? record.status)) &&
      Number(parsePayload(record.payload).packageRecordId) === packageRecordId,
    );
    if (activeAssignment) return res.status(409).json({ error: "الباقة التنظيف مرتبطة بتخصيص نشط بالفعل" });
  ...
  if (kind === "container_movement") {
    ...
    const containerCode = String(payload.containerCode ?? "").trim();
    if (!containerCode || !movementType) return res.status(422).json({ error: "رقم الباقة التنظيف ونوع الحركة مطلوبان" });
    if (!movementStatus(movementType)) return res.status(422).json({ error: "نوع حركة الباقة التنظيف غير مدعوم" });
    const asset = await findAssetByCode(containerCode);
    if (!asset) return res.status(422).json({ error: "الباقة التنظيف المرتبطة بالحركة غير موجودة" });
    try {
  ...
  if (kind === "container" || kind === "container_asset") {
    ...
    if (!assetCode) return res.status(422).json({ error: "رقم أصل الباقة التنظيف مطلوب" });
    if (await hasDuplicateAssetCode(assetCode)) return res.status(409).json({ error: "رقم أصل الباقة التنظيف مستخدم مسبقًا" });
  }
  try {
  ...
  let created: typeof cleaningOperationsRecordsTable.$inferSelect;
  try {
    created = db.transaction((tx) => {
      const inserted = tx.insert(cleaningOperationsRecordsTable).values({
        kind,
      ...
      if (documentNumberField) {
        ...
        current = tx.update(cleaningOperationsRecordsTable).set({
          reference: documentNumber,
          payload: JSON.stringify(nextPayload),
          updatedAt: new Date().toISOString(),
        }).where(eq(cleaningOperationsRecordsTable.id, inserted.id)).returning().get();
      }
      if (kind === "container_assignment") {
        const assetId = Number(payload.packageRecordId);
        const contractRecordId = Number(payload.contractRecordId);
        const asset = tx.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, assetId)).get();
        if (!asset) throw new Error("أصل الباقة التنظيف غير موجود");
        const contract = tx.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, contractRecordId)).get();
        ...
        tx.update(cleaningOperationsRecordsTable).set({
          ...
          updatedAt: new Date().toISOString(),
        }).where(eq(cleaningOperationsRecordsTable.id, inserted.id)).run();
        tx.update(cleaningOperationsRecordsTable).set({
          status: "reserved",
          payload: JSON.stringify(nextAssetPayload),
          updatedAt: new Date().toISOString(),
        }).where(eq(cleaningOperationsRecordsTable.id, assetId)).run();
        const contractPayload = parsePayload(contract.payload);
        const nextContractPayload = {
          ...contractPayload,
          packageRecordId: assetId,
          assignedCleaningPackageRecordId: assetId,
          assignedSiteRecordId: Number(payload.siteRecordId),
        ...
        tx.update(cleaningOperationsRecordsTable).set({
          payload: JSON.stringify(nextContractPayload),
          updatedAt: new Date().toISOString(),
        }).where(eq(cleaningOperationsRecordsTable.id, contractRecordId)).run();
        tx.insert(cleaningOperationsAuditTable).values({
   ...
   if (kind === "container_movement") {
     try {
       ...
       await db.update(cleaningOperationsRecordsTable).set({ status: "archived", updatedAt: new Date().toISOString() }).where(eq(cleaningOperationsRecordsTable.id, created.id));
       return res.status(422).json({ error: error instanceof Error ? error.message : "تعذر مزامنة حركة الباقة التنظيف" });
     }
   }
  if (kind === "contract") {
    try {
   ...
   if (["payment", "receipt", "expense", "deposit", "bank_deposit", "bank_fee", "invoice", "invoice_return", "payment_return", "transfer", "purchase", "purchase_return", "other_revenue"].includes(kind) && normalizedStatus === "posted") {
    await db.insert(cleaningOperationsRecordsTable).values({
      kind: "ledger_entry",
      status: "posted",
...
router.patch("/admin/cleaning-operations/records/:id", async (req, res) => {
  const adminReq = req as unknown as AdminRequest;
  const id = Number(String(req.params.id));
  const current = await db.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, id)).get();
  ...
  if (current.kind === "container_movement" &&
    ...
    return res.status(422).json({ error: "رقم الباقة التنظيف ونوع الحركة مطلوبان" });
  }
  if (current.kind === "container_movement" && !movementStatus(String(nextPayload.movementType ?? ""))) {
    return res.status(422).json({ error: "نوع حركة الباقة التنظيف غير مدعوم" });
  }
  if (current.kind === "container_movement") {
  ...
  if (current.kind === "container" || current.kind === "container_asset") {
    ...
    if (!assetCode) return res.status(422).json({ error: "رقم أصل الباقة التنظيف مطلوب" });
    if (await hasDuplicateAssetCode(assetCode, id)) return res.status(409).json({ error: "رقم أصل الباقة التنظيف مستخدم مسبقًا" });
  }
  try {
  ...
  const updated = db.transaction((tx) => {
    const next = tx.update(cleaningOperationsRecordsTable).set({
      status: nextStatus,
      payload: JSON.stringify(nextPayload),
      updatedAt: new Date().toISOString(),
    }).where(eq(cleaningOperationsRecordsTable.id, id)).returning().get();
    ...
    if (financialLifecycleKinds.has(current.kind) && current.status === "posted" && nextStatus === "cancelled") {
      const ledgerRows = tx.select().from(cleaningOperationsRecordsTable).all();
      ...
      if (!existingReversal) {
        ...
        tx.insert(cleaningOperationsRecordsTable).values({
          kind: "ledger_entry",
          status: "posted",
          reference: `REV-${id}`,
          payload: JSON.stringify({
...
router.get("/admin/cleaning-operations/financial/core", requireCleaningPackagePermission("financial_reports"), async (req, res) => {
...
router.get("/admin/cleaning-operations/financial/reconciliation", requireCleaningPackagePermission("financial_reports"), async (_req, res) => {
...
router.patch("/admin/cleaning-operations/financial/reconciliation/:id", requireCleaningPackagePermission("financial_reports"), async (req, res) => {
  const adminReq = req as unknown as AdminRequest;
...
router.get("/admin/cleaning-operations/financial/periods", requireCleaningPackagePermission("financial_reports"), async (_req, res) => {
  return res.json(financialPeriods());
});

router.post("/admin/cleaning-operations/financial/periods/:periodKey/close", requireCleaningPackagePermission("financial_reports"), async (req, res) => {
  const adminReq = req as unknown as AdminRequest;
...
router.delete("/admin/cleaning-operations/records/:id", async (req, res) => {
  const adminReq = req as unknown as AdminRequest;
  const id = Number(String(req.params.id));
  const current = await db.select().from(cleaningOperationsRecordsTable).where(eq(cleaningOperationsRecordsTable.id, id)).get();
  if (!current) return res.status(404).json({ error: "السجل غير موجود" });
  ...
  await db.update(cleaningOperationsRecordsTable).set({ status: "archived", updatedAt: new Date().toISOString() })
    .where(eq(cleaningOperationsRecordsTable.id, id));
  await db.insert(cleaningOperationsAuditTable).values({
...
router.get("/admin/cleaning-operations/audit", requireCleaningPackagePermission("container_system_audit"), async (_req, res) => {
  const rows = await db.select().from(cleaningOperationsAuditTable)
    .orderBy(desc(cleaningOperationsAuditTable.createdAt)).limit(100);
  return res.json(rows);
});
