...
3. category: اختر تصنيفاً واحداً مناسباً من هذه القائمة فقط: ["عام", "نصائح", "تأجير الباقات التنظيف", "نقل الأنقاض", "أسعار", "أحياء الرياض", "البيئة", "أخبار"]
...
- اجعل العنوان والملخص مرتبطَين بخدمات تأجير باقات التنظيف الأنقاض ونقل المخلفات في الرياض والسعودية
...
أنت كاتب محتوى متخصص في مجال تأجير الباقات التنظيف ونقل الأنقاض في المملكة العربية السعودية.
...
function analyzeLocalSeo(data: {
    ...
    suggestions.push({
      ...
      current: title.slice(0, 80),
       suggestion: title.includes("الرياض")
         ? `${title} | خدمة تنظيف — اتصل الآن`
         : `${title} | خدمات تنظيف بالرياض — تواصل الآن`,
    ...
    suggestions.push({
      ...
      current: description.slice(0, 100),
       suggestion: description.length < 80
         ? `${description} خدمات تنظيف للمنازل والفلل والمكاتب في أحياء الرياض. اطلب عرضك المناسب واتصل الآن.`
  ...
  if (!ogDescription.trim()) {
    suggestions.push({
      ...
      current: "",
       suggestion: description.slice(0, 200) || "متخصصون في تنظيف المنازل والفلل والمكاتب وجلي الرخام وغسيل المكيفات بالرياض. تواصل معنا.",
...
- المجال: تأجير باقات التنظيف المخلفات ونقل الأنقاض

الخدمات الرئيسية:
- تأجير باقات التنظيف 12 و20 و40 ياردة
