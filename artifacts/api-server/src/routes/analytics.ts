...
function viewWeight(
  _row: Parameters<typeof sourceForRow>[0],
  _googleOrganicWeightEnabled: boolean,
): number {
  // Every count must represent one recorded page-view event. The previous
  // implementation multiplied organic Google traffic by eight, making the
  // dashboard and its conversion rates inaccurate.
  return 1;
}
...
router.post("/track", async (req, res) => {
  try {
    ...
    const referrer =
      typeof body.referrer === "string" ? body.referrer.slice(0, 1000) : "";
    const utmSource =
      typeof body.utmSource === "string" ? body.utmSource.slice(0, 160) : "";
    const utmMedium =
      typeof body.utmMedium === "string" ? body.utmMedium.slice(0, 160) : "";
    const utmCampaign =
      typeof body.utmCampaign === "string"
        ? body.utmCampaign.slice(0, 160)
        : "";
    ...
    const existing = await db
      .select()
      .from(activeVisitorsTable)
      .where(eq(activeVisitorsTable.sessionId, sessionId));
    if (existing.length > 0) {
      await db
        .update(activeVisitorsTable)
        .set({ page, lastSeen: now, deviceType })
        .where(eq(activeVisitorsTable.sessionId, sessionId));
    } else {
      await db
        .insert(activeVisitorsTable)
        .values({ sessionId, page, deviceType, lastSeen: now });
    }
...
router.post("/visitor/heartbeat", async (req, res) => {
  try {
    ...
    const page = typeof body.page === "string" ? body.page.slice(0, 500) : "/";
    const deviceType =
      body.deviceType === "mobile" || body.deviceType === "tablet"
        ? body.deviceType
        : "desktop";
    const clientName =
      typeof body.clientName === "string"
        ? body.clientName.trim().slice(0, 160)
        : null;
    const phone =
      typeof body.phone === "string" ? body.phone.trim().slice(0, 40) : null;
    const lastSeen = isoNow();
    const hasConversationId = Object.prototype.hasOwnProperty.call(
      body,
      "conversationId",
    );
    const hasClientName = Object.prototype.hasOwnProperty.call(
      body,
      "clientName",
    );
...
router.get(
  ...
  requireAdmin,
  requireNonDriver,
  requireSectionPermission("conversations"),
  async (_req, res) => {
    const cutoff = isoAgo(5 * 60 * 1000);
    await db
      .delete(activeVisitorsTable)
      .where(sql`${activeVisitorsTable.lastSeen} < ${cutoff}`);
      ...
      rows
        ...
        .map((row) => ({
          ...
          hasPendingInvitation: Boolean(
            row.invitationMessage && row.invitationCreatedAt,
          ),
        })),
    );
  },
...
router.post(
  "/admin/active-visitors/:sessionId/invite",
  requireAdmin,
  requireNonDriver,
  requireSectionPermission("conversations"),
  async (req, res) => {
    ...
    if (!sessionId || !message)
      return res.status(422).json({ error: "رسالة الدعوة مطلوبة" });
    const [visitor] = await db
      ...
      .returning();
    if (!visitor)
      return res.status(404).json({ error: "الزائر لم يعد متصلاً" });
...
router.get("/visitor/invitation", async (req, res) => {
  ...
  const [visitor] = await db
    ...
    .from(activeVisitorsTable)
    .where(eq(activeVisitorsTable.sessionId, sessionId));
  if (!visitor || !visitor.invitationMessage || !visitor.invitationCreatedAt)
...
router.post("/visitor/invitation/accept", async (req, res) => {
  ...
  const clientName =
    typeof req.body?.clientName === "string"
      ? req.body.clientName.trim().slice(0, 160)
      : "";
  const phone =
    typeof req.body?.phone === "string"
      ? req.body.phone.trim().slice(0, 40)
      : "";
  const service =
    typeof req.body?.service === "string"
      ? req.body.service.trim().slice(0, 160)
      : "";
  if (!sessionId || !clientName || !phone)
    return res.status(422).json({ error: "الاسم ورقم الجوال مطلوبان" });
  const [visitor] = await db
    .select()
    .from(activeVisitorsTable)
    .where(eq(activeVisitorsTable.sessionId, sessionId));
  ...
  await db.insert(messagesTable).values({
    conversationId: conversation.id,
    content: service
      ? `أرغب في الاستفسار عن خدمة: ${service}`
      : "أرغب في التواصل مع الدعم المباشر",
    senderType: "client",
  });
...
router.get(
  "/admin/analytics",
  requireAdmin,
  requireSectionPermission("analytics"),
  async (req, res) => {
    try {
      res.set(
        "Cache-Control",
      ...
      await db
        .delete(activeVisitorsTable)
        .where(sql`${activeVisitorsTable.lastSeen} < ${fiveMinAgo}`);
      ...
      const weight = (row: (typeof allRows)[number]) => viewWeight(row, false);
      ...
      const selectedRequests = allRequests.filter(
        (request) =>
          (!period.from || request.createdAt >= period.from) &&
          (!period.to || request.createdAt <= period.to),
      );
      const comparisonPeriod = getComparisonPeriod(period);
      const comparisonRows = comparisonPeriod
        ? rowsIn(comparisonPeriod.from, comparisonPeriod.to)
        : [];
      const comparisonRequests = comparisonPeriod
        ? allRequests.filter(
            (request) =>
              request.createdAt >= comparisonPeriod.from &&
              request.createdAt <= comparisonPeriod.to,
          )
        : [];
      ...
      const conversionSources = [
        ...new Set([
          ...Object.keys(viewSourceCounts),
          ...Object.keys(requestSourceCounts),
        ]),
      ]
        .sort(
          (a, b) =>
            (requestSourceCounts[b] || 0) - (requestSourceCounts[a] || 0) ||
            (viewSourceCounts[b] || 0) - (viewSourceCounts[a] || 0),
        )
        .map((source) => {
      ...
      const orderStatuses = {
        pending: selectedRequests.filter(
          (request) => request.status === "pending",
        ).length,
        inProgress: selectedRequests.filter(
          (request) => request.status === "in_progress",
        ).length,
        completed: selectedRequests.filter(
          (request) => request.status === "completed",
        ).length,
        cancelled: selectedRequests.filter(
          (request) => request.status === "cancelled",
        ).length,
      };
      const servicePerformance = Object.values(
        ...
        .map((row) => ({
          ...
          completionRate:
            row.total > 0
              ? Number(((row.completed / row.total) * 100).toFixed(1))
              : 0,
        }));
      const averageHours = (
        rows: typeof selectedRequests,
        endField: "assignedAt" | "driverCompletedAt",
      ) => {
        const durations = rows.flatMap((request) => {
          const end = request[endField];
          if (!end) return [];
          const hours =
            (new Date(end).getTime() - new Date(request.createdAt).getTime()) /
            3_600_000;
      ...
      const operationalMetrics = {
        assigned: selectedRequests.filter((request) =>
          Boolean(request.assignedAt),
        ).length,
        ...
        averageCompletionHours: averageHours(
          selectedRequests.filter((request) => request.status === "completed"),
          "driverCompletedAt",
        ),
      };
      const selectedUnique = new Set(selectedRows.map((row) => row.sessionId))
        .size;
      const comparisonUnique = new Set(
        comparisonRows.map((row) => row.sessionId),
      ).size;
      const selectedConversion =
        selectedUnique > 0
          ? Number(
              ((selectedRequests.length / selectedUnique) * 100).toFixed(1),
            )
          : 0;
      const comparisonConversion =
        comparisonUnique > 0
          ? Number(
              ((comparisonRequests.length / comparisonUnique) * 100).toFixed(1),
            )
          : 0;

      return res.json({
        activeCount: activeRows.length,
        activePages: activeRows.map((row) => ({
          page: row.page,
        ...
        today: {
          views: weightedViews(todayRows, weight),
          unique: new Set(todayRows.map((row) => row.sessionId)).size,
        },
        week: {
          views: weightedViews(weekRows, weight),
          unique: new Set(weekRows.map((row) => row.sessionId)).size,
        },
        month: {
          views: weightedViews(monthRows, weight),
          unique: new Set(monthRows.map((row) => row.sessionId)).size,
        },
        topPages: ranked(
          countBy(selectedRows, (row) => row.page, weight),
          8,
        ).map(({ label, count }) => ({ page: label, count })),
        topReferrers: ranked(
          countBy(selectedRows, (row) => row.referrer || "مباشر", weight),
          8,
        ).map(({ label, count }) => ({ referrer: label, count })),
        sources: ranked(countBy(selectedRows, sourceForRow, weight), 10).map(
          ({ label, count }) => ({ source: label, count }),
        ),
        orders: {
...
router.delete(
  "/admin/analytics",
  requireAdmin,
  requireSectionPermission("analytics", { adminOnly: true }),
  async (_req, res) => {
