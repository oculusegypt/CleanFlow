...
import { conversationsTable, messagesTable, serviceRequestsTable, packagesTable } from "@workspace/db";
...
const CATEGORY_META: Record<string, { title: string; description: string; emoji: string }> = {
  debris: { title: "باقات التنظيف الأنقاض والهدم", description: "باقات التنظيف 12 و15 و20 و30 ياردة لمخلفات الهدم والترميم", emoji: "🏗️" },
  waste: { title: "باقات التنظيف النفايات والمكابس", description: "باقات التنظيف 6 و10 ياردة ومكابس نفايات كهربائية للمنشآت", emoji: "🚛" },
  contract: { title: "عقود النظافة ورخص بلدي", description: "عقود نظافة إلكترونية معتمدة من أمانة الرياض", emoji: "📋" },
};

interface CleaningPackageData {
  allCleaningPackages: CleaningPackageCard[];
  services: ServiceCard[];
}

function mapDbToCleaningPackageCards(
  dbRows: (typeof packagesTable.$inferSelect)[],
): CleaningPackageCard[] {
  return dbRows
    .filter((c) => c.isActive)
    .map((c) => ({
...
function mapCleaningPackagesToServiceCards(containers: CleaningPackageCard[]): ServiceCard[] {
...
async function fetchCleaningPackageData(): Promise<CleaningPackageData> {
  try {
    ...
    const allCleaningPackages = mapDbToCleaningPackageCards(dbRows);
    return { allCleaningPackages, services: mapCleaningPackagesToServiceCards(allCleaningPackages) };
  } catch {
    ...
    return { allCleaningPackages: [], services: [] };
  }
}

function containerMatchesMessage(container: CleaningPackageCard, message: string): boolean {
...
function getPriceLabel(container: CleaningPackageCard): string {
...
function findCurrentCleaningPackage(
  containers: CleaningPackageCard[],
  message: string,
): CleaningPackageCard | undefined {
...
function detectCatalogService(text: string, cd: CleaningPackageData): string | null {
...
async function getWelcomeMessage(): Promise<BotResponse> {
  ...
  return {
    reply:
      `أهلاً وسهلاً! 👋 أنا المساعد الذكي لـ **${siteName}** — متخصصون في تأجير باقات التنظيف الأنقاض والنفايات ونقل المخلفات بالرياض.\n\nكيف أقدر أساعدك اليوم؟`,
    messageType: "options",
    options: [
...
async function handleMainMenu(message: string, intent: string, state: FlowState, cd: CleaningPackageData): Promise<BotResponse> {
  ...
  if (intent === "prices") {
    const prices = cd.allCleaningPackages.map((c) =>
...
async function handleServiceType(message: string, state: FlowState, cd: CleaningPackageData): Promise<BotResponse> {
  const service = detectCatalogService(message, cd);
  const t = message.toLowerCase();
...
function goToServiceFlow(serviceId: string, existingData: FlowData, cd: CleaningPackageData): BotResponse {
  const category = serviceId === "all" ? null : serviceId;
  const cards = category
    ? cd.allCleaningPackages.filter((container) => container.category === category)
    : cd.allCleaningPackages;
...
function handlePackageSelect(message: string, state: FlowState, cd: CleaningPackageData): BotResponse {
  const t = message.toLowerCase();
  ...
  const visibleCleaningPackages = state.data.containerCategory && state.data.containerCategory !== "all"
    ? cd.allCleaningPackages.filter((container) => container.category === state.data.containerCategory)
    : cd.allCleaningPackages;
  const c = findCurrentCleaningPackage(visibleCleaningPackages, message);
  if (c) {
    const selectedData: FlowData = {
      ...state.data,
      serviceType: CATEGORY_META[c.category]?.title || c.name,
      packageSize: `${c.name}${c.size ? ` - ${c.size}` : ""}`,
...
function handleCollectPhone(message: string, state: FlowState): BotResponse {
  ...
  const {
    serviceType, packageSize, containerPrice, location, name,
  ...
  return {
    ...
    orderData: {
      serviceType, packageSize, containerPrice, location, name, phone,
      appointmentType, scheduledAt, duration, notes, activityType, monthlyEvacuations, packageDetailsText,
    },
...
async function handleConfirm(message: string, intent: string, state: FlowState, cd: CleaningPackageData): Promise<BotResponse> {
  const t = message.toLowerCase();

  if (intent === "confirm" || /تأكيد|ارسل|يلا|ماشي|نعم|اوكي|موافق/.test(t)) {
    const {
      serviceType, packageSize, location, name, phone, activityType,
    ...
    return {
      ...
      messageType: "success",
      orderData: {
        orderId: request.id, phone, name, serviceType, packageSize, location,
...
async function processMessage(message: string, state: FlowState): Promise<BotResponse> {
  ...
  const cd = await fetchCleaningPackageData();
...
async function getOrCreateAiConversation(
  ...
  const requestedId = rawConversationId == null || rawConversationId === ""
    ? null
    : Number(rawConversationId);

  if (requestedId !== null) {
    if (!Number.isInteger(requestedId) || requestedId <= 0) {
      throw new Error("conversationId invalid");
    }
    const [existing] = await db.select({ id: conversationsTable.id })
      .from(conversationsTable)
      .where(eq(conversationsTable.id, requestedId));
    if (!existing) throw new Error("conversation not found");
    return existing.id;
  }

  const clientName = typeof data.name === "string" && data.name.trim()
    ? data.name.trim()
    : "زائر المساعد الذكي";
  const phone = typeof data.phone === "string" && data.phone.trim()
    ? data.phone.trim()
    : "غير محدد";
  const packageName = typeof data.packageSize === "string" && data.packageSize.trim()
    ? data.packageSize.trim()
    : null;
  ...
  await createNotification({
    title: "محادثة جديدة",
    message: `بدأ ${clientName} محادثة مع المساعد الذكي`,
    type: "chat",
...
router.post("/ai/chat", async (req, res) => {
  ...
  let convId: number;
  try {
    convId = await getOrCreateAiConversation(conversationId, flowState.data);
  } catch (error) {
    const reason = error instanceof Error ? error.message : "";
    if (reason === "conversationId invalid") {
      return res.status(400).json({ error: "معرّف المحادثة غير صحيح" });
    }
    if (reason === "conversation not found") {
  ...
  const responseData = response.flowState?.data;
  const conversationDetails: {
    clientName?: string;
    phone?: string;
    packageName?: string;
  } = {};
  if (responseData?.name?.trim()) conversationDetails.clientName = responseData.name.trim();
  if (responseData?.phone?.trim()) conversationDetails.phone = responseData.phone.trim();
  if (responseData?.packageSize?.trim()) conversationDetails.packageName = responseData.packageSize.trim();
  if (Object.keys(conversationDetails).length > 0) {
    await db.update(conversationsTable)
      .set({ ...conversationDetails, updatedAt: new Date().toISOString() })
      .where(eq(conversationsTable.id, convId))
      .catch(() => {});
  }
  ...
  // A successful AI order must be visible to the support team as a structured
  // confirmation, not only as the bot's plain-text reply.
  if (response.messageType === "success" && response.orderData?.orderId) {
    const orderData = response.orderData;
    const confirmationContent = `تم تأكيد طلب الخدمة رقم #${orderData.orderId} من داخل المساعد الذكي`;
    await db.insert(messagesTable)
      .values({
      ...
      .catch(() => {});
    await db.update(conversationsTable)
      .set({ lastMessage: confirmationContent, updatedAt: new Date().toISOString() })
      .where(eq(conversationsTable.id, convId))
      .catch(() => {});
  }
