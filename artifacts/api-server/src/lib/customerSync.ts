import { db, cleaningOperationsRecordsTable, serviceRequestsTable } from "@workspace/db";
...
/**
 * Makes every order source visible in CleaningPackage Operations as a customer and
...
export async function syncCustomerFromRequest(request: ServiceRequest) {
  ...
  const savedCustomer = customer
    ? (await db.update(cleaningOperationsRecordsTable).set({
      ...
      }).where(eq(cleaningOperationsRecordsTable.id, customer.id)).returning())[0] ?? customer
  ...
  if (location && location !== "غير محدد") {
    ...
    if (!site) {
      site = (await db.insert(cleaningOperationsRecordsTable).values({
        kind: "customer_site",
        status: "active",
