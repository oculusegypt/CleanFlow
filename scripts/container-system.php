 ...
 *
 * This file is included by api-index.php and uses PHP 7.4+,
...
function hsEnsureSchema(PDO $pdo): void {
    ...
    try { $pdo->exec("ALTER TABLE container_system_records ADD COLUMN operation_key TEXT"); } catch (Throwable $migrationError) { /* already exists */ }
    try {
    ...
    } catch (Throwable $jsonError) { /* legacy SQLite without JSON1 */ }
...
function hostingerContainerSystemRoute(PDO $pdo, string $path, string $method, array $input): void {
    // The current Node API exposes the public contract under
    // /admin/cleaning-operations, while older Hostinger builds used the
    // internal /admin/container-system name. Normalize both names here so
    // the deployed PHP API stays compatible with the current frontend.
    if (str_starts_with($path, '/admin/cleaning-operations')) {
        $path = '/admin/container-system' . substr($path, strlen('/admin/cleaning-operations'));
    }
