 ...
 * prerender.mjs
 * ──────────────
 * يولّد ملفات HTML ثابتة (SSG) لجميع صفحات المدونة والخدمات والباقات التنظيف
 ...
 *   dist/public/cleaning-packages/[seo_slug]/index.html
 */
...
const SEO_DEFAULTS = {
  companyName: "خدمات التنظيف بالرياض",
  description: "خدمات تنظيف المنازل والفلل والشقق والمكاتب في الرياض، مع تنظيف ما بعد البناء وجلي الرخام وغسيل المكيفات.",
  phone: "0554498403",
  address: "الرياض",
  city: "الرياض",
  region: "منطقة الرياض",
...
<head>
  ...
  <meta property="og:site_name" content="${esc(siteBrandName)}" />
  ...
  <!-- Schema.org JSON-LD — available to crawlers in the source HTML -->
  ${schemasHtml}
  ...
  <style>
    ...
    html.js-enabled #seo-static-page-content { display: none !important; }
  </style>
...
function dynamicHomeSchema() {
  ...
  const business = {
    ...
    "alternateName": [siteBrandName, siteCompanyName],
  ...
  return [
    business,
    {
      ...
      "url": publicUrl("/"),
      "name": siteBrandName,
      "alternateName": [siteCompanyName, siteBrandName],
      "inLanguage": "ar",
...
function updateIndexSeo(html) {
  ...
  const upsertHeadTag = (source, pattern, value) => {
    ...
    if (!matches.length) return source.replace("</head>", `${value}\n</head>`);
    let index = 0;
    return source.replace(pattern, () => index++ === 0 ? value : "");
  };
  let next = html;
  ...
  next = upsertHeadTag(next, /<link[^>]+rel=["']canonical["'][^>]*>/gi, `<link rel="canonical" href="${esc(publicUrl("/"))}" />`);
  next = upsertHeadTag(next, /<meta[^>]+property=["']og:site_name["'][^>]*>/gi, `<meta property="og:site_name" content="${esc(siteBrandName)}" />`);
  next = upsertHeadTag(next, /<meta[^>]+property=["']og:title["'][^>]*>/gi, `<meta property="og:title" content="${esc(title)}" />`);
  next = upsertHeadTag(next, /<meta[^>]+property=["']og:description["'][^>]*>/gi, `<meta property="og:description" content="${esc(description)}" />`);
  next = upsertHeadTag(next, /<meta[^>]+property=["']og:url["'][^>]*>/gi, `<meta property="og:url" content="${esc(publicUrl("/"))}" />`);
  next = upsertHeadTag(next, /<meta[^>]+property=["']og:image["'][^>]*>/gi, `<meta property="og:image" content="${esc(logo)}" />`);
  next = upsertHeadTag(next, /<meta[^>]+property=["']og:image:alt["'][^>]*>/gi, `<meta property="og:image:alt" content="${esc(`شعار ${siteBrandName}`)}" />`);
  ...
  const withStaticCss = withHomeStatic.replace(
    "</head>",
    `\n<style>#seo-static-page-content{display:block}html.js-enabled #seo-static-page-content{display:none!important}</style>\n<script>document.documentElement.classList.add("js-enabled");</script>\n</head>`,
  );
  return withStaticCss;
}
...
// ══════════════════════════════════════════════════════════════════════════════
{
  ...
  const ogImage = `${SITE_URL}/brand-icon.png`;
...
// ══════════════════════════════════════════════════════════════════════════════
// 5.4 صفحة الباقات /cleaning-packages/index.html
// ══════════════════════════════════════════════════════════════════════════════
{
  ...
  savePage("cleaning-packages", renderPage({
    title, description, canonical, ogImage: `${SITE_URL}/brand-icon.png`,
    keywords: "باقات التنظيف بالرياض, باقات تنظيف منازل, باقات تنظيف فلل, حجز خدمة تنظيف",
    schemas: [schema], breadcrumbs: [{ name: "الرئيسية", url: SITE_URL }, { name: "باقات التنظيف", url: canonical }],
    bodyContent: `<article class="prose max-w-none"><h1>باقات التنظيف بالرياض</h1><p class="lead">${esc(description)}</p><ul>${items}</ul><p><a href="/اتصل-الآن">اتصل الآن لحجز الباقة</a></p></article>`
  }));
  console.log("   ✅ صفحة الباقات /cleaning-packages");
}

// ══════════════════════════════════════════════════════════════════════════════
// 5.5 صفحة الخدمات /services/index.html
// ══════════════════════════════════════════════════════════════════════════════
{
  ...
  const schema = {
    "@context": "https://schema.org", "@type": "CollectionPage", name: title,
    description, url: canonical, inLanguage: "ar",
    mainEntity: { "@type": "ItemList", itemListElement: services.filter(s => s.isActive !== false && (s.seoSlug || s.slug)).map((s, index) => ({
      "@type": "ListItem", position: index + 1, name: s.title, url: `${SITE_URL}/services/${encodeURIComponent(s.seoSlug || s.slug)}`
    })) }
  };
  savePage("services", renderPage({
    title, description, canonical, ogImage: `${SITE_URL}/brand-icon.png`,
    keywords: "خدمات التنظيف بالرياض, تنظيف منازل, تنظيف فلل, جلي رخام, غسيل مكيفات",
    schemas: [schema], breadcrumbs: [{ name: "الرئيسية", url: SITE_URL }, { name: "الخدمات", url: canonical }],
    bodyContent: `<article class="prose max-w-none"><h1>خدمات التنظيف بالرياض</h1><p class="lead">${esc(description)}</p><ul>${items}</ul><p><a href="/اتصل-الآن">اتصل الآن لحجز الخدمة</a></p></article>`
  }));
  console.log("   ✅ صفحة الخدمات /services");
}

// ══════════════════════════════════════════════════════════════════════════════
// 5.4 صفحة الاتصال السريع /اتصل-الآن/index.html
// ══════════════════════════════════════════════════════════════════════════════
{
  const canonical = `${SITE_URL}/اتصل-الآن`;
  const phone = sitePhoneCall || sitePhones[0] || "";
  const tel = `tel:${phone.replace(/[^\d+]/g, "")}`;
  const title = `اتصل الآن | ${siteCompanyName}`;
  const description = `اتصل الآن بـ${siteCompanyName} لحجز خدمة تنظيف في الرياض والتحدث مباشرة مع فريق العمليات.`;
  const bodyContent = `
    <article class="prose max-w-none" style="text-align:center">
      <h1>اتصل الآن بـ${esc(siteCompanyName)}</h1>
      <p class="lead">${esc(description)}</p>
      <p><a href="${esc(tel)}" style="display:inline-block;background:#f59e0b;color:#fff;padding:14px 28px;border-radius:10px;font-weight:800">الاتصال بالمنشأة: ${esc(phone)}</a></p>
      <script>window.location.href=${JSON.stringify(tel)};</script>
    </article>
  `;
  const schema = {
    "@context": "https://schema.org", "@type": "ContactPage",
    name: title, description, url: canonical, inLanguage: "ar",
    mainEntity: { "@type": "LocalBusiness", "@id": `${SITE_URL}/#business`, name: siteCompanyName, telephone: phone }
  };
  const html = renderPage({
    title, description, canonical,
    ogImage: `${SITE_URL}/هوية-السهم-كلين.png`,
    keywords: "اتصل الآن شركة تنظيف بالرياض, رقم شركة تنظيف بالرياض, حجز خدمة تنظيف",
    schemas: [schema, breadcrumbSchema([{ name: "الرئيسية", url: SITE_URL }, { name: "اتصل الآن", url: canonical }])],
...
// ══════════════════════════════════════════════════════════════════════════════
// 5.5 صفحة اتصل بنا /contact/index.html
// ══════════════════════════════════════════════════════════════════════════════
{
