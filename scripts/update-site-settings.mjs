...
for (const dbPath of dbs) {
  try {
    ...
    const hasSiteSettings = db
      .prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'site_settings'")
      .get();
    if (!hasSiteSettings) {
      db.close();
      console.warn(`Skipped ${dbPath}: site_settings table is not present`);
      continue;
    }
