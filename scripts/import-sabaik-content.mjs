...
// 2. Import CleaningPackages (packages table)
try {
  ...
  if (fs.existsSync(containersFile)) {
    ...
    const insertCleaningPackage = db.prepare(`
    ...
    tx();
    console.log("✓ CleaningPackages imported successfully into packages table.");
  }
} catch (err) {
    ...
    const tx = db.transaction(() => {
      for (const item of records) {
        insertPage.run({
          ...
          category: item.pageType || "باقات التنظيف",
...
try {
  db.exec(`
    ...
    ('م. فهد السبيعي', 'مدير مشاريع إنشائية', 'تعاملنا مع سبائك الماسة في عدة مشاريع كبرى بالرياض، التزام رائع في مواعيد تسليم الباقات التنظيف وسحبها فور الامتلاء وسرعة استجابة لا مثيل لها.', 5, 1, datetime('now')),
    ('أبو راشد القحطاني', 'صاحب فيلا - حي النرجس', 'طلبت باقة التنظيف 15 ياردة لأعمال الترميم، وصلت في نفس اليوم وتم سحبها بكل سهولة، أسعارهم مناسبة جداً وخدمة راقية.', 5, 1, datetime('now')),
    ...
    ('تأجير باقات التنظيف المخلفات والأنقاض بالرياض', 'باقات التنظيف 6 إلى 30 ياردة للمشاريع السكنية والتجارية مع سرعة في التوصيل والسحب 24/7', '/images/Banner-Big.webp', 'اطلب حاويتك الآن', 0, 1, datetime('now')),
    ('نقل مخلفات الهدم والبناء بأعلى معايير السلامة', 'أسطول شاحنات حديث مجهز لخدمة جميع أحياء الرياض بأسعار تنافسية', '/images/Banner-Big.webp', 'احجز موعد فوري', 1, 1, datetime('now')),
    ('عقود نظافة معتمدة ومكابس نفايات ذكية', 'حلول متكاملة للمنشآت والمطاعم والمصانع لتجديد الرخص وتفريغ منتظم', '/images/Banner-Big.webp', 'تواصل معنا', 2, 1, datetime('now'));

    DELETE FROM company_values;
    INSERT INTO company_values (title, description, icon, "order") VALUES
    ('السرعة والالتزام بالمواعيد', 'توصيل الباقات التنظيف وسحبها خلال 2 إلى 4 ساعات على مدار الساعة طوال أيام الأسبوع.', 'Clock', 0),
    ('أسطول شاحنات حديث ومتنوع', 'باقات التنظيف بمختلف المقاسات من 6 إلى 30 ياردة ومكابس نفايات هيدروليكية متطورة.', 'Truck', 1),
