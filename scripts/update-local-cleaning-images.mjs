...
const db = new Database(process.env.DB_PATH || join(ROOT, "data/sabaik.db"));
...
const updatePackage = db.prepare("UPDATE containers SET image_url = ?, images = ? WHERE id = ?");
...
const updateAll = db.transaction(() => {
  ...
  for (const pkg of packages) {
    ...
    updatePackage.run(image, JSON.stringify([image]), pkg.id);
  }
});
